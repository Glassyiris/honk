    ...
    fn make_node(id: uuid::Uuid, name: &str) -> Node {
        Node { id, name: name.into(), ..Default::default() }
    }
    fn make_group(name: &str, policy: GroupPolicy, ids: Vec<uuid::Uuid>) -> Group {
        Group { name: name.into(), policy, nodes: ids, ..Default::default() }
    }
    ...
    fn udp_pick_prefers_node_with_udp_latency_over_mirror() {
        ...
        alive.record_probe_latency("anytls", ProbeDomain::Tcp, IpVersion::V4, Duration::from_millis(50));
        // trojan: worse TCP, but has real UDP latency.
        alive.record_probe_latency("trojan", ProbeDomain::Tcp, IpVersion::V4, Duration::from_millis(200));
        alive.record_probe_latency("trojan", ProbeDomain::DataUdp, IpVersion::V4, Duration::from_millis(283));

        let udp = m.select_node_for_domain("japan", ProbeDomain::DataUdp, IpVersion::V4);
        assert_eq!(udp.unwrap().name, "trojan", "UDP pick must prefer the node with real UDP latency");
    }
