...
async fn bind_dns_pair() -> (tokio::net::UdpSocket, tokio::net::TcpListener) {
    for _ in 0..64 {
        let tcp = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        match tokio::net::UdpSocket::bind(tcp.local_addr().unwrap()).await {
            Ok(udp) => return (udp, tcp),
            Err(error) if error.kind() == io::ErrorKind::AddrInUse => {}
            Err(error) => panic!("bind UDP DNS fixture: {error}"),
        }
    }
    panic!("no shared TCP/UDP loopback port available");
}
...
async fn truncated_udp_answer_retries_over_tcp() {
    ...
    let (udp, tcp) = bind_dns_pair().await;
...
async fn observed_bootstrap_lookup_preserves_source_lineage_and_cancellation() {
    ...
    let (server, tcp) = bind_dns_pair().await;
    ...
    let peer = tokio::spawn(async move {
        ...
        let mut truncated = addr_response(&buf[..size], &[]);
        truncated[2] |= 0x02;
        server.send_to(&truncated, peer).await.unwrap();
        let (mut stream, _) = tcp.accept().await.unwrap();
        let mut query = vec![0; stream.read_u16().await.unwrap() as usize];
        stream.read_exact(&mut query).await.unwrap();
        let response = addr_response(&query, &["127.0.0.9".parse().unwrap()]);
        stream.write_u16(response.len() as u16).await.unwrap();
        stream.write_all(&response).await.unwrap();
        server
    });
    ...
    let lookups: Vec<_> = events
        ...
        .filter_map(|(context, event)| match event {
            FlowEvent::Dns(lookup) => {
                ...
                let transport = if lookup.status == "succeeded" {
                    "tcp"
                } else {
                    "udp"
                };
                assert_eq!(lookup.upstream_transport, Some(transport));
                assert_eq!(lookup.carrier_transport, Some(transport));
                Some(lookup)
            }
            _ => None,
        })
