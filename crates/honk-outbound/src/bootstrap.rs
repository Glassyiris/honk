...
pub async fn resolve_with(
    ...
    if let Ok(contents) = tokio::fs::read_to_string("/etc/hosts").await {
        ...
        if !addrs.is_empty() {
            ...
            if let Some(mut observation) = LookupObservation::start(host, "UNKNOWN", None) {
                observation.finish(Ok(addrs.iter().copied()));
            }
            return Ok(addrs);
        }
    }
...
impl BootstrapResolver {
    ...
    async fn query_family(&self, host: &str, qtype: u16) -> io::Result<Vec<IpAddr>> {
        ...
        #[cfg(feature = "native-api")]
        let child = observation.as_ref().map(LookupObservation::child);
        let result = {
            let operation = std::pin::pin!(async {
            ...
            {
                match child {
                    Some(child) => child.scope(operation).await,
    ...
    async fn query_raw(
        ...
        host: &str,
        qtype: u16,
        #[cfg(feature = "native-api")] observation: &mut Option<LookupObservation>,
    ) -> io::Result<Vec<u8>> {
        ...
        if let Some(observation) = observation {
            observation.data.upstream_transport = Some("tcp");
            observation.data.carrier_transport = Some("tcp");
        }
...
pub async fn query_ech_config(host: &str) -> io::Result<Option<(Vec<u8>, u32)>> {
    ...
    let child = observation.as_ref().map(LookupObservation::child);
    let result = {
        let operation = std::pin::pin!(resolver.query_raw(
            host,
            QTYPE_HTTPS,
            #[cfg(feature = "native-api")]
            &mut observation,
