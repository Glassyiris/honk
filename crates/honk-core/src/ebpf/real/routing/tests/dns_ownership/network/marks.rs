...
fn compiled_direct_mark_index_matches_canonical_table_and_absent_actions() {
    isolated(|| {
        ...
        for (dscp, outbound, mark, must, indexed) in [
            ...
            assert_eq!(observed.trace.decision, observed.decision);
            assert_eq!(
                observed.trace.flags,
                honk_ebpf_common::ROUTE_TRACE_VERSION
                    | honk_ebpf_common::ROUTE_TRACE_ENABLED
                    | honk_ebpf_common::ROUTE_TRACE_COMPLETE
            );
        ...
        assert_eq!(miss.trace.decision, miss.decision);
    });
}
