...
Build with `--features native-api` and configure `experimental.native_api` for independent observations and controls on `127.0.0.1:9527`: bounded flows/logs/DNS history, typed probes, provider status/refresh, per-network Selector control, exact connection closure and genuine suspend/resume. Captured `.dae` sources remain authoritative, with offline validation and opt-in administrator source/Group PATCH transactions; there is no configuration database. Native and Clash share transient mode only when native is enabled. Bearer authentication is required unless anonymous loopback is explicitly enabled; an optional trusted local UI directory is served without downloads. Full kernel transparency, automatic-policy overrides, node/provider CRUD/geodata management and embedded/real doona integration remain unavailable. See [native settings](doc/en/reference/experimental.md#native_api) and the [API contract](doc/en/reference/api.md#native-api-m1).
