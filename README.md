...
VLESS share links default to `vless_mode=auto`: ordinary UDP uses native VLESS on ports 53/443 and Single XUDP elsewhere; Vision uses Single XUDP. `udp=0` disables packets without changing TCP. Explicit `native`, TCP-only `legacy`, `uot-v2`, `h2mux`, `h2mux-padded`, `xudp`, and `mux-cool` modes remain available; only the explicit mux modes pool logical TCP streams.

Base Vision rejects UDP/443; `flow=xtls-rprx-vision-udp443` permits it. VLESS Encryption supports the unpooled `legacy`, `auto`, `native`, and `xudp` modes without flow. No mode negotiation, fallback, or first-packet replay occurs; local policy/size refusals are health-neutral. The local official-server fixture covers native/automatic UDP, XUDP, Vision UDP443, and encrypted packet exchanges. See the [node reference](doc/en/reference/nodes.md#modes) for wire, lifecycle, and import rules.
