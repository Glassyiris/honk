...
- (**Experimental, OFF by default** — enable with `HONK_UDP_POST_DECISION_OFFLOAD=1`. Default-off rationale: the userspace first packet exits via the engine's own ephemeral socket while offloaded packets continue via kernel NAT/forwarding, so the server-visible 5-tuple changes at the switch — unsafe for stateful UDP such as QUIC handshakes or game/VoIP sessions.) UDP flows can additionally be offloaded **after** the userspace decision fully converges. Unlike TCP, UDP carries no sequence state the kernel must track from the first byte, so when a flow's control-plane decision lands on `direct` (routing + mode override + group selection + a successful first send), the control plane rewrites the flow's published conn_state meta in place (`RoutingMeta::set_offloaded_direct`: outbound normalized to `Direct`, tproxy mark cleared, bit 57 set) and the `lan_ingress` established-UDP path passes subsequent packets straight through. Mode semantics match the route-time policy: `Rule` (or clash API disabled) offloads a converged-`direct` decision, `Direct` normalizes every non-`must`/non-`block` decision to `direct`, `Global` keeps every non-`must` flow in userspace. Port 53 is never offloaded — the DNS hijack depends on the DnsController seeing every packet (structurally, port-53 UDP has no conn_state to flag). The write happens only at the Ready commit of a direct endpoint: never during initialization or cold-URLTest staggering, and never for a proxied decision. The endpoint is deliberately kept until the idle reaper retires it so the in-flight first reply is still delivered (single-shot UDP is unaffected); offloaded flows stop appearing in `/connections` and their rx bytes are not counted (rx is accounted at `dae0_ingress`), exactly like `must`-direct. After 120s of silence the conn_state is swept and the next datagram simply repeats the decide-and-offload cycle. The per-flow bit binds at decision time, so flows created before a reload or mode switch keep their cached decision.
