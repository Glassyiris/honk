...
- (**Experimental, OFF by default** — `experimental.udp_nfqueue`, phase-1 skeleton.) The former drop-and-PTO post-decision offload was removed. In its place, Rule-mode non-`must` direct UDP flows that cannot be route-time offloaded are *staged*: `lan_ingress` publishes the handoff, stamps the conn_state `Pending` with a random `decision_cookie`, tags the skb with `NFQUEUE_PENDING_MARK` (bit 29) and passes it into the stack, where the `honk_nfqueue` inet prerouting chain (priority -250 — after defrag, before conntrack) queues it to userspace workers (`crates/honk-nfqueue`, raw NETLINK_NETFILTER, no `nft` CLI). The phase-1 worker commits `DirectActive` via `EbpfBackend::offload_udp_flow` and NF_ACCEPTs the original skb with the pending bit cleared — the first datagram keeps the client's own 5-tuple end-to-end (no userspace socket, no PTO), and subsequent packets pass through the kernel on the offload bit. Packets of a still-`Pending` flow re-enter NFQUEUE until the commit lands, so a staged flow never flips to the TPROXY path mid-decision. Every queued packet gets exactly one verdict via an RAII guard (uncommitted guards fail closed to NF_DROP); the ruleset is installed atomically and only honk-owned objects are ever removed. The pipeline is brought up before datapath admission opens (listeners → rules → self-check → `DATAPATH_FLAG_NFQ_READY`) and any bring-up failure aborts startup; with the feature disabled the datapath is byte-for-byte the pre-NFQUEUE one. An unresolved fragmented ClientHello is still dropped for retransmit rather than decided on an IP-only guess (the sniffer session keeps the fragments); the proxy/block commit paths, the pending-flow pool, and QUIC-driven staging decisions land in the next phase.
