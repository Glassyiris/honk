...
AnyTLS owns the reusable pool in its runtime generation, and shutdown makes that
pool terminal; reload retires only the old generation before the replacement starts.
Only `Ready` and `AlreadyReady` are warm successes. Direct, other non-AnyTLS, and
