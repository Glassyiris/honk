...
| 从 `.sub` 升级 | 首次启动取得实例锁后，honk 依次在 `<data_dir>/.sub`、`/var/share/honk/.sub`、`./.sub` 中查找第一个私有的旧存储，复制已启用订阅的正文；状态数据库中已有的正文优先。如果某个已启用订阅的正文无法读取、超过 8 MiB 或会使合计超过 32 MiB，其余正文照常复制，但导入不会记录，旧存储也不会删除；honk 记录警告，下次启动时重试。完整复制后才记录在数据库中，不会重复执行；随后 honk 删除该目录中的所有 `*.sub` 与 `.*.tmp`，目录为空时一并删除；其他旧位置保持不变。此时旧进程已经退出，它最后写入的正文与其他正文一样被复制。此后再启动旧版本时，它找不到 `.sub`，会重新拉取。 |
