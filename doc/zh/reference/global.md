...
| `nfqueue_enable` | `nfqueue_enable` | `true` | 将有歧义的 LAN 转发 UDP 原始包保留在 NFQUEUE，直到用户态得到终态决策。需要真实 eBPF 后端；mock/不带 `ebpf` 的启动或队列/nftables 前置检查失败时，honk 记录 warning，仅在本进程关闭该功能且不改写配置。修改后需重启。新配置应使用此字段；已弃用的 `experimental.udp_nfqueue.enabled` 写法仍接受并给出迁移 warning。 |
