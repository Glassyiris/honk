...
| 裸标量内的撇号，如 `log_file: /tmp/don't`，或已开启的流量参数引号 | 裸值内的撇号是词法错误 | `/tmp/don't` 是字面值。普通表达式的引号只在词法单元开头，或紧接引号外的 `(`、`,` 时开启；声明还识别紧接分隔冒号的带引号值（见下文），顶层 `include` 识别相邻的带引号路径。流量规则中未在同一物理行闭合的引号产生带位置的 `unterminated-quote` 错误；请闭合引号，不要依赖花括号恢复结构。 |
...
| 声明冒号后不加空格的带引号值，如 `bind:'tcp+udp://[::]:53530'` 或 `tag:'https://example.com/sub'(agent)` | `:` 两侧的空白可省略 | 标量及带标签条目接受此写法，单、双引号均可。首个引号外分隔冒号开启值；裸 URL、路径及 User-Agent 后缀中的后续冒号仍是数据。带引号标签内部的冒号不会被当作分隔符。流量/DNS 路由表达式与 include 路径保留各自的词法规则。 |
