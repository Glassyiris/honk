...
`download.route` 决定每个 geodata 请求（包括校验文件）的出口。`direct` 为默认值，与订阅下载相同，使用 bootstrap resolver 解析并带绕过标记直连主机；`routing` 与用户流量一样遵循路由规则；`group` 始终经过 `group_id` 指定的组，该 ID 即 `GET /groups` 返回的 ID。经规则或组的请求与外部 UI 下载共用同一套路由决策和隧道。路由无法承载的请求按连接错误处理，使这个 URL 失败：组不存在或没有可选成员时 `last_error.code` 为 `group_unavailable`，规则指向 `block` 时为 `route_blocked`，隧道建立失败时为 `connection_failed`。随后 honk 改试下一个 URL，不会回退到直连。启动后不久组内节点仍不可达时，更新就按此处理。经节点下载时，域名由节点出口解析，因此探测目标策略只检查端口和字面 IP 地址。`experimental.native_api.geodata_download_detour` 在启动时按与 URL 相同的规则写入路由：`direct`、`routing` 或组名覆盖通过 API 修改的路由；空值保留通过 API 修改的路由，并删除先前配置文件写入的路由。`external_ui_download_detour` 为空时遵循路由规则，这里的空值则表示配置文件未设置路由。路由不影响 `source`。
