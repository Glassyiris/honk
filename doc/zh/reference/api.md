...
暂停不是硬时间上限承诺：清理阶段超期不能丢弃 owner 或宣称暂停成功。Bootstrap DNS 使用异步带 bypass mark 的查询，不回退 libc NSS；订阅 response 取消与专有 runtime teardown 负责关闭并 join HTTP driver。终止关闭不同于 suspend：关闭准入并停止 watcher、detach hooks 后，健康运行中的连接有默认 5 秒 drain grace，随后强制取消并 join；故障退出不承诺该 grace。原生 HTTP 自身另有 5 秒 graceful drain，未完成的清理仍须保留所有权。
