...
启动失败输出及正常日志中的配置提示包含固定 schema 路径 `setting` 和数字 `source`：后者是本次解析来源表中从零开始的序号，`0` 表示入口来源。可用的物理行号 `line` 和字节列号 `byte_column` 从一开始；没有位置时省略这些字段。任意配置值和源文件路径仍不回显。旧列表写法的提示会指明具体全局字段，例如 `global.udp_check_dns`；请逐项加引号，不要用一对引号包住整个逗号列表。
