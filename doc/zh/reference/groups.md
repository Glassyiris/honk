...
只读比较与上述切换决策分离：实际响应使用对称 10% 近似等价容差，不使用完成数成熟度或毫秒下限。合格可靠性与已知方向保护保持有效；被保持的成员或真实测量取舍仍可能未确认。合格指标分别持有作用域身份，未知或交集中消失的传输方向不会使响应 cohort 失效。不一致的响应 cohort 请求预算内可比响应流量（包括对照观测），不是带宽测速，详见[有条件的验证结论](../design/groups.md#有条件的验证结论)。
