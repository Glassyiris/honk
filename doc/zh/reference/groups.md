...
晋升至多比较四个挑战者与同一个固定现任，使用双方精确目标、至多八个规范顺序选取且等权的共同目标，或同一当前配置探测 cohort。无关聚合均值、setup 与预热不是证明。四个共同 15 秒块在各自起点后 60 秒到期，每个指标要求双方在共同块内合计各有四个不同 reporter。方向优势要求至少提升 10%，其他已知方向均未退化超过 10%，响应变慢不超过 10%，且合格终态可靠性不低于现任。未知方向保持未知；一升一降报告为权衡，不以最佳方向分数掩盖。成对响应与合格可靠性优势仍参与既有保持门槛判定。常量不可配置，详见[生命周期设计](../design/groups.md#score-评分与生命周期)。
