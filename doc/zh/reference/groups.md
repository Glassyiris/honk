...
晋升比较按需求确定大小的评估集（3–25 个成员，见[有条件的验证结论](../design/groups.md#有条件的验证结论)）内所有合格挑战者与同一个固定现任，使用双方精确目标、至多八个按规范顺序选取且响应支持已合格的共同目标，或同一当前配置探测 cohort。共同目标先判定支持资格再应用数量上限，不能按测量值挑选；跳过尚未合格或被上限截断的已匹配目标时，只保留部分局部支持，响应缺口仍待解决，全组比较仍未确认。方向指标在同一组已选响应目标上取交集。业务证明使用四个共同 15 秒块，在各自起点后 60 秒到期；配置探测按实际周期 `I` 使用四个 `max(15s, 2I)` 块，有效期还受“较弱一侧最近支持加 `max(60s, 2I)`”限制。每个指标仍要求双方各四个不同 reporter，改变周期不能继承旧探测支持。方向优势要求至少提升 10%，其他已知方向均未退化超过 10%，响应变慢不超过 10%，且合格终态可靠性不低于现任。未知方向保持未知；一升一降报告为权衡，不以最佳方向分数掩盖。成对响应与合格可靠性优势仍参与既有保持门槛判定。不新增 Score 调节项，详见[生命周期设计](../design/groups.md#score-评分与生命周期)。

只读比较与上述切换决策分离：实际响应使用对称 10% 近似等价容差，不使用完成数成熟度或毫秒下限。合格可靠性与已知方向保护保持有效；被保持的成员或真实测量取舍仍可能未确认。实际可靠性低于所选成员、且双方都有四个有效完成的不合格成员，无需配对即可在评估集覆盖中结清，但其指标不被称为近似等价。合格指标分别持有作用域身份，未知或交集中消失的传输方向不会使响应 cohort 失效。不一致的响应 cohort 请求预算内可比响应流量（包括对照观测），不是带宽测速，详见[有条件的验证结论](../design/groups.md#有条件的验证结论)。
