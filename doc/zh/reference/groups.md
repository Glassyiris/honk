...
冷探索对不超过 4 个成员的组会覆盖每个候选。更大的组只采样 `ceil(sqrt(n)) + 1` 个候选（不超过组大小），并周期性重新检查尝试次数最少的非当前成员；周期检查每进行 `2n` 次 Score 选择触发一次，间隔限制在 16–64 次选择。这样小组能完整覆盖，大组也不会把路由预算耗在一次性探测上。

所有历史计数、加权和与权重都采用固定 30 分钟半衰期的指数衰减。新鲜失败证据会绕过当前节点的 `0.01` utility 切换余量；组合衰减失败证据低于一个样本的 `0.01` 后，普通余量会重新生效。setup 和首响应延迟取衰减加权均值。吞吐证据仅接纳终态成功、收发双向均非零、持续至少 1 秒，且主导方向传输至少 64 KiB 的交换；合格窗口以主导方向字节数除以总持续时间汇总，避免双计请求与响应，也不让短小突发影响排名。这些半衰期和门槛是固定实现常量，不可配置。
