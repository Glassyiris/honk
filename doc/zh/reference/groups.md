...
比较优先使用合格的联合响应时间块交集，原始成对不利证据仍可否决支持。集中成对验证在每个组／网络／地址族至多保留一个绑定目标的对照／挑战者轮次，限制为 45 秒、16 次匹配选择机会与八次可选预留尝试。已经合格但时间块不一致的 cohort 则通过已有近期顺序采样分配有额度的观测，不重新等待攒满四份额度，普通转发提供对照。两条路径均保留业务额度、在途上限和无关目标焦点，不重放请求。只读覆盖与阻塞字段解释尚未解决的缺口，不暴露目标身份。
