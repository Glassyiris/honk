...
每个保留的 `(group, TCP/UDP, target family)` 作用域固定成员数 `n`、冷启动额度 `B`（四个及以下覆盖全部，否则 `ceil(sqrt(n)) + 1`）和赚取周期 `q = clamp(2n,16,64)`。原始业务开始数 `N` 为可选工作提供额度：已支出试用 `V` 加未开始预留 `R` 始终不超过 `B + floor(N/q)`；未花费的已赚额度最多为八。`N` 包括被选中的试用，但重试、clone 和嵌套重复归因不能放大计数，根计数对嵌套组去重。时间、读取、新目标与证据过期不产生额度；保留作用域在 reload／成员变化后保持额度。节点专属工作在 DNS／准入等待前开始支出，与物理／逻辑 reporter 开始分离；工作开始前释放最后引用会退款，开始后取消不退款。恢复重试单独计数，不花可选预算。有进展候选可在额度和在途限制内至多获得八次集中选择。试用不成为普通现任；真实失败保留指数退避与三连败普通排除，退避到期仍允许恢复试用。
...
晋升至多比较四个挑战者与同一个固定现任，使用双方精确目标、至多八个按规范顺序选取且响应支持已合格的共同目标，或同一当前配置探测 cohort。共同目标先判定支持资格再应用数量上限，不能按测量值挑选；跳过尚未合格或被上限截断的已匹配目标时，只保留部分局部支持，响应缺口仍待解决，全组比较仍未确认。方向指标在同一组已选响应目标上取交集。业务证明使用四个共同 15 秒块，在各自起点后 60 秒到期；配置探测按实际周期 `I` 使用四个 `max(15s, 2I)` 块，有效期还受“较弱一侧最近支持加 `max(60s, 2I)`”限制。每个指标仍要求双方各四个不同 reporter，改变周期不能继承旧探测支持。方向优势要求至少提升 10%，其他已知方向均未退化超过 10%，响应变慢不超过 10%，且合格终态可靠性不低于现任。未知方向保持未知；一升一降报告为权衡，不以最佳方向分数掩盖。成对响应与合格可靠性优势仍参与既有保持门槛判定。不新增 Score 调节项，详见[生命周期设计](../design/groups.md#score-评分与生命周期)。

已完成 setup 且已有 TX 的定向 Traffic RX，可在 flow 结束前恢复普通现任保护，但不清除连败、不增加终态成功。首次合格 RX 立即发布，后续按事件驱动且每个 reporter 每秒至多一次；被限频的接收需要等待下一次可发布 RX 或终态。四个有效 useful 完成与新鲜业务 RX 可取得 60 秒资格租约，租约内 RX 可续期而不扩大证据计数。失败／reload 使租约失效；低于完成数门槛的候选不能仅凭 RX 恢复已过期租约。普通选路的这些规则与 `scoreVerification` 报告的活跃可用性 cohort 保持独立。
...
采样在共享预算内解决可用性、响应、普通资格和恢复缺口。新鲜等价 cohort 须在资格／恢复缺口也关闭后才停止强制采样，缺少传输证据则等待真实负载。`scoreVerification` 区分临时转发、已观测可用性、比较范围／有效期、证据问题 `question` 和等待原因 `waitReason`。`localComparison` 只涉及活跃挑战者，不会升级不完整的全局覆盖；聚合读取检查已保留 IPv4/IPv6 等待状态，不花费额度。详见 [API 验证](./api.md#score-验证信息)。这些是可撤销的经验结论，不是最优候选识别或未来网络行为的保证。
