...
启用 `use_host` 后，构建 generation 时只读取一次一个来源。默认的 `/etc/hosts` 解析器索引精确名称及别名；`hosts_file` 切换到 OxiDNS 兼容的精确、domain 后缀、regexp 和 keyword 规则，精确与最长后缀匹配优先于有序的 regexp 和 keyword 匹配。查询处理不执行文件 I/O。
