...
`src/parser/mod.rs` 负责文件级 `include` 和有序顶层分派。`cursor/syntax.rs` 统一判定作用域、块头和语句边界；`cursor.rs` 为引用源文本的词法单元及所有嵌套片段建立一次索引，记录块内语法和下一同级位置。`Dispenser` 只遍历该索引，`read.rs` 共享值与表达式读取，各节读取器负责解码设置。包含文件仍以入口目录为基准，拒绝越界与循环；各读取器整块跳过未知嵌套块，节点和订阅的兼容包装块仍会遍历。

语法在注释和花括号分类之前提供引号策略，只有边界判定者通知 lexer 开始新的声明头。因此紧凑同级块之后使用实际的后续块头；已有值的条目、标量或 User-Agent 中的原始 `{}`，以及紧随其后的已索引起始花括号都不会凭空产生同级声明。订阅块的标签范围只记录一次，读取器不再独立分类。

物理行片段保留已接纳或已忽略、紧凑块或普通块的区别。忽略的语句头不更新续接状态；路由括号保护状态可跨越同源路由块，但不泄漏到其他作用域，并在来源边界重置。路由读取器从这些行片段组装语义表达式；订阅字段独立于块头范围保留完整起始词法单元。迭代器的消费进度或克隆不会改变语法。

读取器返回类型化错误，只有最外层尝试发布终止诊断。上下文诊断先补充归属，再与已有归属的诊断进入同一个队列；输出路径不再充当控制标志。按来源的前缀最大值排序、调用方已有诊断、根级注释与 EOF 的顺序，以及原始引号错误来源均保持不变。
