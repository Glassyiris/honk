...
`src/experimental.rs` 的 `ExperimentalConfig` 持有 `clash_api`、`native_api` 与 `cache_file`。原生设置严格校验、独立启用且均需重启；默认关闭的 `native-api` feature 提供可见用户态观测、共用计数/RSS/cgroup 历史，以及真实 `.dae` 启动来源的读取、离线校验和受控写入/reload operations。`.dae` 仍是唯一配置权威，原文披露与写许可分别显式开启，含 API 凭据的源始终省略正文且只读；M3b 组控制和未实现管理能力仍关闭。弃用的 `udp_nfqueue` 块仍只用于迁移，将 `enabled` 复制到 `GlobalConfig::nfqueue_enable` 并发出 warning。
