...
普通首次选择仍使用组内相对 utility。不再合格的现任可立即替换；仍合格但业务失败尚未恢复的现任也可绕过保持。恢复依据是严格晚于每个适用 cell 最后失败的实际业务 RX：可以来自已完成 setup 且已有 TX 的定向 Traffic reporter 的已接受活跃进展，也可以来自双向成功终态。活跃进展还受失败／reload 失效边界与已捕获 cell incarnation 约束。仅 setup、单向 TX、探测、预热和取消本身不能制造恢复；取消也不会撤回已经发生的有效 RX。历史失败数、连败与退避仍只按既有终态规则结算。

对于已训练、合格且已恢复的现任，启发式至多提出四个合格挑战者，均与该固定现任比较。晋升证据独立维护：双方精确目标观测，否则选取至多八个共同目标，先按规范键顺序选取再读取数值，各目标等权；或使用同一当前配置探测 cohort。无关聚合均值、setup 与预热不是比较证明。每个指标要求双方在共同的 15 秒块内合计各有四个不同 reporter；保留四块，每块在起点后 60 秒到期，而非最后事件后 60 秒。失败、reload、cell incarnation 和当前探测 cohort 边界都会使支持失效。
...
业务 RX 上报独立于吞吐窗口：首次符合条件的非零 RX 立即发布，随后每个 reporter 每秒至多一次。普通单向 TX 回调不补发被限频的接收事件，也没有定时器把静默变成进展。共享源首次确认成功的非零发送可以补认其串行发送开始后已交付的回包，沿用实际 RX 时间而不是确认时间；仍在排队、失败或已退役的发送不能授予这份证据。失败后落在限频间隔内的 RX，在其他情况下仍可能需要等待下一次可发布的 RX 或终态，不保证一个自然秒内恢复。活跃发布更新已观测可用性，但不增加 Beta 终态，也不清除连败。
...
每个保留的 `(group, TCP/UDP, target family)` 预算固定首次成员数 `n`、冷启动额度 `B`（四个及以下覆盖全部，否则 `ceil(sqrt(n)) + 1`）和赚取周期 `q = clamp(2n,16,64)`。原始业务开始数为 `N`、已花费可选试用为 `V`、未开始预留为 `R` 时，始终满足 `V + R <= B + floor(N/q)`；未花费的已赚额度最多为八。`N` 包括被选为试用的原始业务，但重试、clone、嵌套重复计数、读取、时间流逝、证据过期和新目标均不能产生额度。每个经过的作用域对原始业务只计一次，根业务计数对整个嵌套路径去重。保留作用域在成员变化和 reload 后保持额度；删除组或重启才结束这段生命周期。

预留只在节点专属工作开始时支出。最后一个未开始引用被释放，或未开始预留失效时退款；已经开始后的取消绝不退回已花额度。相关 UI 重定向、DNS 地址重试和 TCP 替代尝试在选路前携带原始业务身份，不申请可选试用，不能把已选试用改标签当作免费恢复。恢复尝试保留原始身份，单独计数但不支出可选预算。试用通过与调度和 API 等待原因共用的上下文判定，处理未解决的可用性、响应、普通资格或恢复问题；有进展的候选最多获得八次集中选择，仍受额度和有界在途工作限制，无进展则轮转。新鲜且已解决或近似等价的 cohort，只有资格／恢复缺口也关闭后才停止强制采样；缺失传输证据等待真实负载。真实失败保留 5 分钟至 6 小时退避，只有业务成功降低连败；试用不获得已提交现任保护，三连败排除也不能永久封死退避到期后的恢复验证。
...
结论在最弱支持证据跌破完整置信门槛时失效（此处最多 60 秒）；作为依据的失败排除更早失效时相应提前。业务失败与提交 reload 撤销近期可用性确认，但不清空长期可靠性。API 读取即时重算有效性，不推进采样或计数；已授权 Apply 利用既有有界历史累计确认、过期/矛盾、验证选择和确认耗时。一次确认可以仅指探测比较，业务仍处于临时状态；详见 [API 语义](../reference/api.md#score-验证信息)。
...
失败、提交 reload 或至少 60 秒的进展间隔会重置 cohort。一个恢复接收的 reporter 不能复活之前四份信用；存续 reporter 可用真正新增的 RX 各重新贡献一次，但仍受严格的失败／reload 时间边界和原始 cell incarnation 限制。终态与活跃上报共用去重状态，可按实际 RX 时间补发被限频的合格 RX，不能使用清理时间，也不能跨越已过期的支持窗口。中性取消保留事实 RX 而不增加成功；迟到的成功仍结算长期终态，但不能刷新陈旧可用性。已观测可用不授予普通选路资格，也不清除连败：即使可用性／响应证据已支持比较，有界试用仍会处理资格和恢复缺口。
