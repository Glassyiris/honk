...
当前选择的基线在每次选择时重新读取，而不是保留它胜出时的旧值。因此已退化的当前节点可以被替换；这与 sing-box `Select()` 行为一致。若当前节点的最新样本是合成失败占位符，则跳过滞后——刚失败的当前节点会被立即替换。

探测或拨号失败会追加一个 10 秒合成占位样本。节点的真实历史与移动平均被保留（占位样本不进入平均值），但最新样本为合成的候选排在所有有真实测量的候选之后，直到下一次真实成功——这就是防止不稳定节点凭一次走运探测重回第一的防抖保护。
...
每个节点最近一次真实 TCP 延迟样本每 60 秒写入 `cache.db`；启动时只恢复不超过 24 小时的样本。存活性从不由缓存恢复。合成 10 秒占位样本使节点在选择中沉底，但带有标记，不显示在历史中，不进入移动平均，也不会作为最近真实样本持久化。
