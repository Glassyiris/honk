...
预留只在节点专属工作开始时支出；释放最后一个未开始引用或使未开始预留失效会退款，开始后取消不退款。UI 重定向、DNS 重试和 TCP 替代尝试保留原始业务身份，不能产生试用额度或把已有试用重标为免费恢复。每个组／网络／地址族 cadence 至多保留一个绑定精确目标、固定对照与挑战者的验证轮次。普通比较先攒足剩余 reporter 需求，再交替提供不收费的普通对照与有额度的挑战者工作；初始冷可用性和单步恢复不必等待四份额度。只有新原始业务真正 begin 才推进轮次。每轮至多 45 秒、16 次匹配目标的选择机会或八次可选预留尝试，暂存等待也计入上限；失败、reload、incarnation 丢失或同目标对照变化也会使其失效。无关目标正常转发，不替换当前焦点。缺少流量仍保持未确认，完成或无进展后按有界近期顺序轮转。真实失败保留 5 分钟至 6 小时退避，除非真正的作用域内 RX 打开恢复；试用不取得已提交现任保护。
...
请求响应对齐工作之前，Score 先对有界挑战者集合求新鲜共同时间块交集，再要求交集内双方各有四个不同 reporter；选择只依据身份、时间与资格，不依据测量值。合格联合支持可以立即对齐不同的成对历史，无需等待额外样本或旧证据过期。原始成对证据仍用于晋升与不利证据否决，并继续约束完整结论的指纹与有效期；方向指标使用相同的最终响应目标 cohort。只有联合交集不足才请求有额度的对照／挑战者响应验证，缺失比较对、候选上限与部分目标覆盖仍明确保留。不新增探测、不复制请求，也不保证任意负载都能完成确认。

对于双方已经合格、但彼此时间块不一致的响应 cohort，复用已有的预算内近期顺序采样，而不是固定一个挑战者轮次。只集中刷新一对节点会反复产生互不重叠的时间块。每次获准的对齐试用只需一份可用额度，不重新等待攒满四份；普通转发提供对照观测。退避、在途上限、无关目标的现有轮次及证据资格／有效期均不变，真实负载不足时仍可保持未确认。
