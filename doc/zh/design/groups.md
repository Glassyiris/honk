...
请求响应对齐工作之前，Score 先对全部被覆盖的合格挑战者（轮转成员只在已有响应支持时加入）求新鲜共同时间块交集，再要求交集内双方各有四个不同 reporter；选择只依据身份、时间与资格，不依据测量值。合格联合支持可以立即对齐不同的成对历史，无需等待额外样本或旧证据过期。原始成对证据仍用于晋升与不利证据否决，并继续约束完整结论的指纹与有效期；方向指标使用相同的最终响应目标 cohort。只有联合交集不足才请求有额度的对照／挑战者响应验证，缺失比较对与部分目标覆盖仍明确保留。不新增探测、不复制请求，也不保证任意负载都能完成确认。
...
大组在有界评估集内比较。评估集大小 k 随真实业务变化：原始业务开始数按五分钟半衰期衰减，再折算到 30 分钟证据半衰期；在 `q = 16` 下赚得的可选开始额度，一半用于让每个挑战者保持四个有效完成，记折算值为 D，则 k = 1 + ⌊D/128⌋，限制在 3–25（每分钟 46 条业务时约为 16）。当前视图超过 k 时，评估集包含首选 utility 最高的 k − 1 个成员（平局先按主导配置探测延迟，再按 NodeId）、本次决策的胜者，以及 1 个轮转成员，轮转成员按 NodeId 顺序依次访问其余成员，每位 10 分钟。只有已授权 Apply 存储评估集；它最多每五分钟重新排名一次，扩大在刷新时生效，只有 k 下降至少 2 时才收缩，过滤视图或重试视图中暂缺的成员保留原位，只读读取从不改变它。被覆盖成员必须已比较或已结清，结论才完整，且完整结论至少需要一个被覆盖的挑战者。只要已有成员合格，排名成员就在首次合格时加入覆盖（从不依据测量值），并保留到被排名移出或 reload 重新选择成员为止；此前它属于已评估成员并接受可选工作，但不能阻碍结论，加入后资格失效会重新打开结论，而不是缩小覆盖。尚无成员合格时，所有排名成员均被覆盖。轮转成员与仍在取得资格的排名成员取得证据后参与否决与区间，但不能阻碍完整性或联合对齐。评估集外的成员为未评估，从不算作已比较、已排除或被击败；它们不接受可选工作，探测基线保留在统计中但不进入比较单元，挑战者离开评估集的活跃验证轮次会失去见证。普通首选、逃逸与恢复仍考虑全部成员；证明工作受 k 限制，排名本身仍与组规模线性相关。评估轮转按时间推进，与按 begin 推进的验证 recency 相互独立。
