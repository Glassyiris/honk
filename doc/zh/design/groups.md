...
冷启动只有一份组/网络/目标地址族预算：四个及以下覆盖全部，否则为 `ceil(sqrt(n)) + 1`。后续时间/次数/退化机会共享一次预留，用于尚未解决的可用性、响应、普通资格或失败恢复缺口。有真实进展的有希望候选得到最多八次试用选择的集中验证；取消或无进展会轮转，不能永久占住验证。这让低流量大组有机会在历史衰减前完成资格确认。新鲜且已经解决或近似等价的候选，只有在资格和恢复缺口也关闭后才停止强制采样；过期、矛盾或成员/探测变化再产生缺口。粗粒度边界仍为 30 秒或 `clamp(2n,16,64)` 次选择，退化加速至少要求距前次试用经过 16 次选择。真实失败保留 5 分钟至 6 小时退避，只有业务成功降低连败；试用不获得现任保持保护，三连败排除也不能永久封死退避到期后的恢复验证。
