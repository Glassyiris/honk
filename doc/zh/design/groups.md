...
重载时，配置未变化的节点会把现有 `NodeRuntime` 转移给替代 generation，其中包括存活的 AnyTLS、VLESS H2MUX/Mux.Cool 与 QUIC 状态。旧 generation 不再接受新的预热工作，活跃流则正常排空。周期健康探测只测量冷节点，不会预热 generation；按需 Clash 延迟测试则先在临时 runtime 中预热任意冷可复用 session 或 QUIC client，计时后关闭，因此组扫描不会新增每成员常驻 transport 状态。
