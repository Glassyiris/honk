...
业务 RX 上报独立于吞吐窗口：首次符合条件的正向 RX 立即发布，随后每个 reporter 每秒至多一次。只有后续非零 RX 回调能再次发布活跃进展；没有定时器，单向 TX 也不会补发被限频的接收事件。因此，失败后落在限频间隔内的 RX 可能需要等待下一次可发布的 RX 或终态，不保证一个自然秒内恢复。活跃发布更新已观测可用性，但不增加 Beta 终态，也不清除连败。
