...
- The process-owned DNS log records actual completed client outcomes once, including known source socket/ingress and parseable refusal/error results; diagnostic calls and background refresh duplicates are excluded. It defaults to 512 records/8 MiB without subscribers, evicts whole oldest records and invalidates affected cursors. Restart clears it; suspend retains it. Query/cache/log response projection is capped at 262144 bytes and fails rather than silently clipping answer sets. Cache and log pages end before the entry that would exceed the cap and return `next_cursor`; only an entry that fits no page fails.
