...
| `mimalloc` | yes | Pulls in `mimalloc` and `libmimalloc-sys` and installs mimalloc as the `honk-core` binary allocator. Before starting Tokio, startup enables cross-thread abandoned-page reclaim; on Linux, it also disables transparent huge pages for the process. |
