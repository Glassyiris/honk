...
- `src/parser/` — `mod.rs` owns file-aware `include` and ordered root dispatch. `cursor/syntax.rs` classifies scope, headers and statement boundaries once; `cursor.rs` indexes source-backed tokens and all nested segments with their body grammar and next-sibling positions. `Dispenser` only traverses that index, while `read.rs` shares value/expression access and section readers decode their settings. Includes remain entry-relative, confined and cycle-checked; unknown nested blocks are skipped by their owning readers, and node/subscription compatibility wrappers still traverse.
  Grammar supplies lexical quote policy before comment/brace classification; only the boundary owner signals a new declaration head to the lexer. Compact siblings therefore select the actual following header, while populated entries, raw scalar/User-Agent `{}` and immediately following indexed openers cannot invent a sibling. Subscription block tag spans are recorded once rather than independently reclassified by the reader.
  Physical-line segments retain accepted/ignored and compact/braced distinctions. Ignored heads do not update continuation; routing parentheses carry between same-source routing roots without leaking into other scopes, and reset at source boundaries. Routing readers assemble semantic expressions from those line segments. Subscription fields retain full opening-token bytes independently of their header spans; iterator consumption or cloning cannot alter grammar.
  Readers propagate typed errors and only the outer attempt publishes a terminal diagnostic. Contextual notices receive attribution before entering the same queue as already-owned notices; output paths are not control flags. Source-local prefix-maxima ordering, caller prefixes, root comment/EOF ordering and original quote-error provenance remain intact.
