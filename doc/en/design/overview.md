...
- `src/experimental.rs` — `ExperimentalConfig` owns `clash_api`, `native_api`, and `cache_file`. Native settings are strict, independent and restart-required. Default-on `native-api` provides userspace observations, bounded histories, source-owned Group PATCH and main-entry administration, plus verified-geodata activation; its listener remains default-off. `.dae` remains authoritative; automatic overrides, native mode and full kernel transparency stay gated. Default-off `native-ui` embeds licensed pinned doona assets without runtime fetching. The deprecated `udp_nfqueue` block remains migration-only and copies `enabled` to `GlobalConfig::nfqueue_enable` with a warning.
...
- **Score isolation and reasons:** Health filtering and business-target family remain independent. Score qualifies observed failure risk without rewarding historical sample volume, then compares fresh configured-probe and target quality. All-size validation shares a coarse group/network/family time/count budget; trials do not become committed incumbents. Real-flow streaks retain backoff and three-failure exclusion. Exact and aggregate evidence use separate 4,096-entry LRUs; overlapping counts are not added. Reload preserves admitted business outcomes while revoking recent confirmation and probe baselines. Public summaries and counters stay scope-limited and never expose raw scorer target keys; see [group design](./groups.md#score-scoring-and-lifecycle).
...
- **Conditional verification surface:** Score proxy objects additionally publish aggregate provisional/usable state, comparison basis, missing evidence and remaining validity with the same readonly selected member. `/stats` adds bounded group/network verification transition and exposure counters; neither surface exports raw target keys or a probability of optimality. See [API verification](../reference/api.md#score-verification).
