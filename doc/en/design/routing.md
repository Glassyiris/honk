...
The group-shared OR snapshot described above keeps a group alive while any eligible leaf is alive. Clash `Global` and `Direct` overrides still cannot bypass a `must` or `block` result. See the [datapath design](./datapath.md) for the exact redirect and drop paths.
