...
`native_api.rs` owns its listener, 64-connection JoinSet, one one-second sampler and native tracker consumer through joined shutdown. Header budget is five seconds; idle I/O and stalled writes have independent 30-second deadlines, so healthy SSE can outlive 30 seconds. Shutdown has one five-second HTTP connection grace budget. `observation.rs` retains process identity and bounded flow/catalog/event/telemetry/log/DNS stores independently of clients. Actual TCP/UDP producer hooks capture immutable partial evidence; accepted publication emits generation events under existing barriers. Native-only final handoffs still avoid legacy rule recomputation. Traffic/memory histories share the sampler, skip missed ticks and retain at most 600 points/600 seconds; unreadable RSS/cgroup fields remain null. Backend-owned datapath observations are partial, never full kernel transparency or kernel-memory accounting.

`native_api/handlers.rs` registers each resource's methods once; the shared security boundary still precedes method and resource validation. `flows/record.rs` owns typed summaries, inputs and six evidence-step forms; retention counts owned heap capacity, and JSON projection stays at the wire boundary. Core lifecycle and mode commands return typed outcomes rather than HTTP errors or JSON.
