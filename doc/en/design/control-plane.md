...
`native_api.rs` owns its listener, 64-connection JoinSet, one one-second sampler and native tracker consumer through joined shutdown. Header budget is five seconds; idle I/O and stalled writes have independent 30-second deadlines, so healthy SSE can outlive 30 seconds. Shutdown has one five-second connection grace budget. `observation.rs` owns process identity and bounded flow/catalog/event stores. Actual TCP/UDP producer hooks capture immutable partial evidence without changing routing or cleanup; accepted reload publication emits generation events under existing barriers. Native-only final handoffs still avoid legacy rule recomputation. No kernel transparency, traffic-history or log rings are claimed.
