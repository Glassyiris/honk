...
- `src/subscription.rs` owns fetching, parsing and FD-relative raw-body persistence. `subscription/supervisor.rs` keeps current authorization and deferred/refresh-eligible scheduling in one provider record; `supervisor/startup.rs` restores cached bodies and applies the shared five-second startup grace. Startup and steady-state share one supervisor state. In-flight work retains its captured revision and publication acknowledgement independently of replacement scheduling; pause/shutdown joins it, rather than reconciliation discarding a possibly committed reply.
...
Typed probes pin config/group/registry ownership, resolved addresses and measurement dimensions before network attempts, within fixed job/member/rate/deadline bounds. Provider status is supplied by the subscription supervisor, and refresh success waits for authorized runtime publication. Rule dictionaries and resolve-none simulations pin router/config/generation together; parser source spans provide editor locations without exposing paths. Native logs capture audited structured fields rather than formatted console/Clash text. Logs and native events both resume with ready, then replay. One settings owner validates a full merge before changing any recorder; accepted explicit activation resets overrides, whereas provider/network refresh preserves them.
...
Suspend/resume enters the same bounded coordinator and control-command owner as source work. Successful suspend closes datapath admission and NFQUEUE readiness, drains held verdicts, closes exact visible TCP/UDP owners and accepted pre-ID TCP tasks, then joins ingress/listener, DNS, health/probe/warm, subscription and protocol-driver work. Listener socket-map references are cleared; bare/ready pools are emptied. Subscription fetches use an owned reqwest runtime, so pause joins its hidden async drivers and started blocking resolver jobs, not merely the visible fetch futures. Pending publication replies are explicitly rejected/drained before acknowledging pause.

`control/lifecycle/teardown.rs` owns shared network cleanup with or without a listener epoch, including partial startup and failed resume. Standalone DNS supervisors retain failures from already-reaped children and propagate them through joined teardown; intentional shutdown cancellation is neutral, but an owner panic cannot produce successful suspension.

Network maintenance belongs to `RuntimeEpoch` and is recreated on resume. The cache delay writer is process-owned, continues periodic snapshots across suspension, and is joined only at terminal shutdown; one-time cache restoration is never repeated on resume.

The API, instance, operation store, accepted sources/configuration, routing/hosts artifacts, group manager, counters, caches and histories remain. Owned programs/maps/hooks may stay loaded/attached with admission closed and TC passing through. Resume binds fresh listeners and creates fresh terminal runtime/task owners while retaining process descriptor/carrier ceilings and group state. It rebuilds from accepted in-memory artifacts, reconciles the paused interface watcher and verifies routing/listener/queue readiness before reopening. Disk edits require a separate activation; cancelled data and old connections are never replayed or restored.

Resume restores DNS query acquisition before reopening pending/NFQUEUE/datapath admission. Active health/probe scheduling remains after ingress opens; a DNS readiness failure leaves the candidate fenced for the existing joined cleanup.

DNS diagnostics, probes, provider refresh, source activation and mode/group mutations conflict during known suspension or transitions before new network work; actual stopped/failed or otherwise unavailable owners report unavailability. Retained observations, recorder settings and cache administration do not need fresh network owners. Shutdown takes priority and preserves the existing accepted-flow drain grace before joined teardown. Cleanup/fence ambiguity is fatal, never reported as successful suspension. Stop deadlines are failure thresholds, not permission to detach unfinished work. Mock operation success does not establish real-kernel forwarding correctness.

Subscriptions use bypass-marked Hyper connections on their owned runtime; response cancellation stops each driver, and runtime teardown joins it before pause completes. Bootstrap resolution uses asynchronous marked DNS, not libc NSS. The health-check owner's five-second drain deadline still retains and joins any admitted work before returning a deadline error; a child failure during late cleanup takes precedence.

When native is enabled, the engine and Clash share one transient mode/target/origin owner: startup and accepted explicit activation (including no-op) reset rule/settings; provider/network refresh and suspend/resume preserve them. A backend mode-reset failure retains previous mode/source: before commit it rejects activation; after routing commit it reports committed-degraded and fences admission. Settings still reset at accepted commit. Global mode retains stable identity and fails closed if that target disappears. With native disabled, existing Clash cached/default mode behavior is unchanged. Native mode and automatic-policy overrides remain gated.

## Clash API and cache DB

The optional Clash-compatible axum server is a userspace view and mutation surface over shared engine handles; endpoint details are in the [API reference](../reference/api.md). Connection tracking starts when an API binds or any group enables `interrupt_connections`; exact transport closure also works without a dashboard. Selector writes serialize with accepted manager replacement, write both networks from Clash and display TCP. The optional SQLite `cachedb` persists per-network Selector choices, delay samples and optional DNS answers; mode persists only with native disabled. Relative paths prefer an existing `global.data_dir` file, then `/var/share/honk`, then an existing config-relative file; missing databases are created below `global.data_dir`. See the [experimental reference](../reference/experimental.md).
