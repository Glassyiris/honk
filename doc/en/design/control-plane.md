...
`native_api.rs` owns its listener, 64-connection JoinSet, one one-second sampler and native tracker consumer through joined shutdown. Header budget is five seconds; idle I/O and stalled writes have independent 30-second deadlines, so healthy SSE can outlive 30 seconds. Shutdown has one five-second connection grace budget. `observation.rs` owns process identity and bounded flow/catalog/event/telemetry stores. Actual TCP/UDP producer hooks capture immutable partial evidence without changing routing or cleanup; accepted reload publication emits generation events under existing barriers. Native-only final handoffs still avoid legacy rule recomputation. Traffic/memory histories share the sampler, skip missed ticks and retain at most 600 points/600 seconds without clients; disabled histories release buffers on restart. RSS/cgroup readings come from actual files and remain null when unknown. No kernel transparency, kernel-memory accounting or native log rings are claimed.
...
For a captured `.dae` startup, `native_api/config.rs` owns the accepted source snapshot; `config/coordinator.rs` serializes source writes and API/SIGHUP reloads before disk parsing. The daemon, not an HTTP connection, owns admitted work through real runtime reload and subscription reconciliation; `operations.rs` retains bounded replay/results and emits `operation.updated`. Offline validation reads authorized local dependencies without network, filesystem mutation, workers or generation publication. PUT validates the complete overlay, rechecks target/dependencies, then atomically replaces and syncs the file before 202 and real reload admission. External editors remain uncoordinated across the final-check/rename window; a post-rename directory-sync error reports written bytes with unconfirmed durability, not rollback. Rejected activation preserves the old accepted snapshot but not old disk bytes; no-op publication can accept comments without a generation change, and committed-degraded retains the new snapshot/generation while reporting failure. Existing publication lock order and subscription fences remain authoritative. Native settings and API credentials require local edits/restart; M6 source writes do not open the M3b group-control gate.
