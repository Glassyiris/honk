...
Reservations are spent only when node-specific work begins. Releasing or invalidating the last unbegun reservation refunds it; begun cancellation never refunds. UI redirects, DNS retries and TCP replacements retain the original business identity and cannot mint trials or relabel an existing trial as free recovery. Each group/network/family cadence retains at most one target-bound validation run with a fixed control and challenger. Ordinary comparison banks the remaining reporter demand, then alternates uncharged ordinary control work and funded challenger work; initial cold availability and one-step recovery need not wait for four credits. Only new original business begins advance turns. Runs end after at most 45 seconds, 16 matching offers or eight optional reservation attempts, including staged waits; failure, reload, incarnation loss or a same-target reference change also invalidates them. Unrelated targets forward normally without displacing that focus. Missing traffic stays inconclusive, and completed/no-progress runs rotate by bounded recency. Real failures retain 5-minute-to-6-hour backoff unless genuine scoped RX opens recovery; trials never acquire committed-incumbent protection.
...
Before requesting response-alignment work, Score intersects fresh shared blocks across the bounded challenger set and qualifies four distinct reporters per side within that intersection. Selection depends on identity/time/qualification, not measured values. Qualified joint support can therefore resolve different pair histories without waiting for extra samples or expiry. Original pair evidence remains authoritative for promotion and adverse-evidence vetoes, and still bounds the full claim's fingerprint and expiry. Directions use the same final response target cohort. Only an insufficient joint intersection requests funded response validation for control and challengers; missing pairs, candidate caps and partial target coverage remain explicit. No extra probes, copied requests or arbitrary-load completion guarantee are introduced.

Already-qualified but misaligned response cohorts use the existing budget-funded recency sampler instead of a fixed-challenger run. Concentrating new observations on one pair can repeatedly create disjoint time blocks. Each admitted alignment trial needs one available credit, not a new four-credit bank; ordinary forwarding supplies control observations. Backoff, in-flight limits, unrelated-target runs and evidence qualification/expiry remain unchanged. Sparse offered traffic can still leave comparison unconfirmed.
