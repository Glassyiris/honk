...
Ordinary first choice still uses group-relative utility. An ineligible incumbent can be replaced immediately; an eligible incumbent with an unresolved business failure also bypasses holding. Recovery uses actual business RX strictly newer than each applicable cell's last failure: either accepted live progress from a targeted Traffic reporter with setup and TX, or a successful bidirectional terminal outcome. Live progress also respects the failure/reload invalidation fence and captured cell incarnation. Setup-only work, TX-only activity, probes, warm-up and cancellation alone cannot manufacture recovery; cancellation does not retract already accepted RX. Historical failure counts, streaks and backoff still change only through the existing terminal settlement rules.

For a trained, eligible, recovered incumbent, at most four eligible challengers proposed by the heuristic compete against that fixed incumbent. Promotion evidence is separate: paired exact-target observations, otherwise at most eight common targets selected in canonical order before inspecting values and weighted equally, or the same current configured-probe cohort. Unrelated aggregate averages, setup and warm-up are not comparison proof. Each metric requires four distinct reporters on each side in shared 15-second blocks; four blocks are retained, and each expires at block start plus 60 seconds, not 60 seconds after its last event. Failure, reload, cell incarnation and current probe-cohort fences invalidate support.
...
Each retained `(group, TCP/UDP, target family)` budget freezes its initial member count `n`, cold allowance `B` (all members up to four, otherwise `ceil(sqrt(n)) + 1`) and earning period `q = clamp(2n,16,64)`. With `N` original business starts, spent optional trials `V` and outstanding reservations `R` obey `V + R <= B + floor(N/q)`; unspent earned credit is capped at eight. `N` includes an original business assigned to a trial, but retries, clones, nested recounting, reads, elapsed time, evidence expiry and new targets cannot mint currency. Each traversed scope counts the original once; the root business count deduplicates the whole nested path. Retained scopes preserve currency across membership changes and reload; deleting the group or restarting ends that lifetime.

Reservations are spent only when node-specific work begins. Releasing the last unbegun reference or invalidating an unbegun reservation refunds it; begun cancellation never refunds spent credit. Related UI redirects, DNS address retries and TCP replacements identify their original business before selection and do not request optional trials; they cannot relabel an already-selected trial as free recovery. Recovery attempts retain their original identity and are counted separately without optional spending. Trials address unresolved availability, response, ordinary qualification or recovery questions through the same contextual evaluation used for scheduling and API waits; a progressing contender gets at most eight focused selections, still subject to currency and bounded in-flight work. No progress rotates opportunities. Fresh settled or near-equivalent cohorts stop forced sampling only once qualification/recovery gaps close; missing transfer waits for real offered load. Real failures retain 5-minute-to-6-hour backoff; only business success reduces the streak. Trials never acquire committed-incumbent protection, and three-failure exclusion does not permanently prevent backoff-expired recovery.
...
Failure, committed reload or a gap of at least 60 seconds resets that cohort. One returning reporter cannot revive its old four credits; surviving reporters can each contribute again after genuinely new RX, subject to strict failure/reload fences and original cell incarnations. Terminal settlement shares live deduplication and may flush suppressed eligible RX at its actual event time, never cleanup time; it cannot bridge already expired support. Neutral cancellation preserves factual RX without a success, and delayed success still settles its lifetime outcome without refreshing stale availability. Observed usability neither grants ordinary qualification nor clears streaks: bounded trials still address qualification and recovery gaps, even when availability/response evidence already supports a comparison.
