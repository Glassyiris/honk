...
Before requesting response-alignment work, Score intersects fresh shared blocks across every covered eligible challenger (a rotation member joins only with response support) and qualifies four distinct reporters per side within that intersection. Selection depends on identity/time/qualification, not measured values. Qualified joint support can therefore resolve different pair histories without waiting for extra samples or expiry. Original pair evidence remains authoritative for promotion and adverse-evidence vetoes, and still bounds the full claim's fingerprint and expiry. Directions use the same final response target cohort. Only an insufficient joint intersection requests funded response validation for control and challengers; missing pairs and partial target coverage remain explicit. No extra probes, copied requests or arbitrary-load completion guarantee are introduced.
...
Large groups compare within a bounded evaluation set. Its size k follows offered business: original business starts decay with a five-minute half-life, are projected to the 30-minute evidence half-life, and half of the optional starts they earn at `q = 16` must keep each challenger at four effective completions, so with D that projection k = 1 + ⌊D/128⌋ within 3–25 (about 16 at 46 flows/min). When the current view exceeds k, the set holds the k − 1 members with the best first-choice utility (ties broken by the dominant configured-probe latency, then NodeId), the decision's winner, and one rotation member that visits the rest in NodeId order for ten minutes each. Only an authorized Apply stores the set; it re-ranks at most every five minutes, grows at that refresh, shrinks only when k falls by two, keeps members missing from a filtered or retry view, and never changes on readonly reads. Covered members must be compared or resolved for a complete claim; the rotation member contributes vetoes and ranges once it has evidence but cannot block completion or joint alignment. Members outside the set are unevaluated, never compared, excluded or beaten; they receive no optional work, keep probe baselines in their stats but no comparison cells, and an active validation run whose challenger leaves the set loses its witness. Ordinary first choice, escape and recovery still consider every member; proof work is bounded by k while ranking itself stays linear in group size. Evaluation rotation is time-based and separate from begin-driven validation recency.
