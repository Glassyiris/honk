...
Whole-candidate coverage resolves a member without a pair in two cases: a recent failure exclusion, or reliability dominance — the member is outside the eligibility band, and it and the selection both have four effective completions while its observed reliability is lower. The advantage rule can never credit such a rival. The resolution is scoped to eligible members: an unpaired member's metrics are not measured equivalent or nonregressing. It lasts only while both sides keep four effective completions, and that lapse bounds the claim's validity; decaying layer weights can still make the member eligible earlier, which revokes the claim before that bound, as it already does for a compared member leaving the band. A resolved member keeps its existing recovery and validation trials, so it may also be pending.
