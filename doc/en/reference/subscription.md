...
| Upgrade from `.sub` | Once the instance lock is held, the first start copies the bodies of enabled subscriptions from the first private legacy store it finds in `<data_dir>/.sub`, `/var/share/honk/.sub` and `./.sub`, keeping bodies already in the state db. If an enabled subscription's body is unreadable, over 8 MiB, or would pass 32 MiB in total, the other bodies are still copied, but the import is not recorded and the store is not removed; honk warns and tries again at the next start. A complete copy is recorded and not repeated, and honk then unlinks every `*.sub` and `.*.tmp` in that directory and removes it if empty; other legacy locations are left alone. The previous process has exited by then, so a body it wrote last is copied like the others. An older binary started afterwards finds no `.sub` and fetches again. |
