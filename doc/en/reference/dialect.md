...
| An apostrophe inside a bare scalar, `log_file: /tmp/don't`, or an opened traffic argument quote | lexer error for the bare apostrophe | `/tmp/don't` is literal. Ordinary expression quotes open only at token head or immediately after unquoted `(` or `,`; declarations additionally recognize quoted values immediately after their separator colon (below), and root `include` recognizes adjacent quoted paths. A traffic quote that does not close on its physical line fails with located `unterminated-quote`; close the quote rather than relying on brace recovery. |
...
| A quoted value without a space after the declaration colon, `bind:'tcp+udp://[::]:53530'` or `tag:'https://example.com/sub'(agent)` | whitespace around `:` is optional | Accepted in scalar and tagged-entry declarations, with either quote delimiter. The first unquoted separator colon opens the value; later colons in bare URLs, paths and User-Agent suffixes remain data. Quoted tags keep their interior colons opaque. Traffic/DNS routing expressions and include paths retain their separate lexical rules. |
