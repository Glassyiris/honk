...
The finite startup allowance covers every candidate up to four, otherwise `ceil(sqrt(n)) + 1`. Subsequent validation opportunities exist after 30 seconds, `clamp(2n,16,64)` selections, or observed degradation after at least 16 selections since the last trial, but are spent only on unresolved availability, response, ordinary qualification or recovery gaps. A progressing promising contender gets at most eight focused trial selections; no-progress/cancelled work rotates fairly. Triggers share a `(group, TCP/UDP, target family or none)` budget; Peek and new targets cannot mint opportunities, and fresh equivalent cohorts stop forced sampling once qualification and recovery gaps also close. Exploration does not acquire ordinary incumbent protection. Real failures retain exponential backoff and three-failure normal exclusion, while backoff-expired recovery trials remain possible.
...
Sampling resolves availability, response, ordinary qualification and recovery gaps within the shared budget, with at most eight focused trials for a progressing contender. Fresh equivalent cohorts stop forced sampling once qualification and recovery gaps also close; missing transfer waits for real offered load. Additive `scoreVerification` summaries distinguish provisional forwarding, observed usability, limited comparisons, evidence scope/expiry and next action; see [API verification](./api.md#score-verification). These are retractable empirical conclusions, not a guarantee of best-arm identification or future network behavior.
