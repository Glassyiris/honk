...
Cold exploration samples every candidate for groups of up to four members. Larger groups sample `ceil(sqrt(n)) + 1` candidates, capped by the group size, and periodically revisit the least-tried non-incumbent; periodic checks occur every `2n` Score selections, clamped to 16–64. This keeps small groups fully covered without making a large group spend its routing budget on one-time probes.

All historical counters and weighted sums decay exponentially with a fixed 30-minute half-life. Fresh failure evidence bypasses the incumbent's `0.01` utility margin; once combined decayed failure evidence falls below `0.01` of one sample, the ordinary margin applies again. Latency is a decayed weighted mean, not a fixed-alpha EWMA. A throughput sample qualifies only when a successful exchange moves bytes in both directions, lasts at least 1 second, and moves at least 64 KiB in its dominant direction. Qualifying samples pool dominant-direction bytes over elapsed time; smaller or shorter exchanges may still affect reliability and latency but not throughput. These constants are not configurable.
