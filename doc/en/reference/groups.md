...
Promotion considers at most four challengers against one fixed incumbent, using paired exact targets, at most eight canonically selected response-qualified common targets, or the same current configured-probe cohort. Common-target qualification precedes the cap without selecting by measured value; skipped unqualified or capped matched targets retain only partial local support, with the response gap and global comparison still unconfirmed. Directional metrics intersect over the same selected response cohort. Business proof uses four shared 15-second blocks expiring at block start plus 60 seconds; configured probes use four blocks of `max(15s, 2I)` for their actual interval `I`, with expiry additionally capped by the weaker side's latest support plus `max(60s, 2I)`. Each metric still requires four distinct reporters per side, and changed cadence cannot inherit prior probe support. Directional gain requires at least 10% improvement, no greater than 10% regression in another known direction, response no more than 10% slower, and equal-or-better qualified terminal reliability. Unknown directions stay unknown; opposing changes are reported as a tradeoff rather than hidden by a best-direction score. Paired response and qualified reliability gains still participate in the existing hold-margin decision. No additional Score tuning field is introduced; see [the lifecycle design](../design/groups.md#score-scoring-and-lifecycle).

Readonly comparison is separate from that switching decision: actual response values use symmetric 10% practical equivalence, not completion maturity or a millisecond floor. Qualified reliability and known directional protections remain; a held member or a genuine measured tradeoff can still be unconfirmed. Qualified metrics own separate scope identities, so unknown or discarded transfer directions cannot invalidate a response cohort. Incompatible response cohorts request budgeted comparable response traffic, including control observations, rather than bandwidth testing. See [conditional verification](../design/groups.md#conditional-verification).

Targeted Traffic RX after setup and TX can restore ordinary incumbent protection before the flow ends, without clearing failure streaks or adding terminal successes. The first eligible RX publishes immediately; subsequent publication is event-driven and capped at once per second per reporter, so a suppressed receive needs another publishable RX or terminal settlement. Four effective useful completions plus fresh business RX can earn a 60-second qualification lease; RX within that lease can renew it without inflating evidence counts. Failure/reload invalidates the lease, and below-threshold candidates cannot regain an expired lease from RX alone. These ordinary-selection rules remain independent of the live availability cohorts reported by `scoreVerification`.
...
Sampling resolves availability, response, ordinary qualification and recovery gaps within the shared budget. Fresh equivalent cohorts stop forced sampling once qualification/recovery gaps also close; missing transfer waits for real offered load. `scoreVerification` distinguishes provisional forwarding, observed usability, comparison scope/expiry, the evidence `question` and `waitReason`. `localComparison` concerns only active challengers and does not upgrade incomplete global coverage; aggregate reads inspect retained IPv4/IPv6 waits without spending credit. See [API verification](./api.md#score-verification). These are retractable empirical conclusions, not a guarantee of best-arm identification or future network behavior.
