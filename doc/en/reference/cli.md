...
| `admin reset` | Deletes the password-mode administrator from the state db under `--data-dir`, and a legacy `native-api/admin.json` that no start has imported yet, so the next start opens setup again. | Refused while any honk-core, mock mode included, has that state db open. |
...
The outer per-node budget also allows two seconds for joined QUIC teardown plus a one-second scheduling margin. Cleanup time is not a latency sample and must not erase already-completed sibling probe results.
