...
| `store_subscribe` | `store_subscribe` | `true` | Persist each last valid subscription body in the state db, `data_dir/state/honk.db`, for startup and reload recovery. A change requires restart. |
...
| Subscription store | Bodies live in `<data_dir>/state/honk.db`. The first start imports the first private legacy `.sub` found in `<data_dir>/.sub`, `/var/share/honk/.sub`, `./.sub`, and removes its files ([subscription reference](./subscription.md#fetch-persistence-and-recovery)). |
