...
All group writes remain explicitly disabled by the current milestone decision. Per-network Selector mutation awaits a shared control/reload/persistence/warm-owner cutover; JSON Patch awaits the M6 lossless file-authority transaction. No memory-only PATCH or synthetic interruption is provided; automatic override also remains gated by the unresolved combined-network response contract.
...
Published events are `stream.ready`, `runtime.updated`, `flow.updated`, `flow.gap` and `generation.changed`; no operation events are fabricated while operations are unavailable. Generation events originate only at accepted publication, not reload receipt. Events contain bounded safe IDs/state, not packet bodies or raw configuration. Flow/event retention is memory-only, not a durable log.
