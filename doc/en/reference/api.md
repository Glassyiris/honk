...
Build with `--features native-api` and enable [`experimental.native_api`](./experimental.md#native_api). `--no-default-features --features native-api` works without Clash. The M1 base API and delivered M2–M6 resources are documented below; M3b group controls remain gated. `.dae` remains the configuration authority, with opt-in source administration rather than a SQLite configuration store.

The frozen contract is [api-standardize cb8ac07c6520b7fb08539cc0b7701695f5a07992](https://github.com/Zakkaus/api-standardize/tree/cb8ac07c6520b7fb08539cc0b7701695f5a07992). Its `base` profile is implemented, not `full_transparency`. Runtime, connections, recorded flows, nodes, groups, events, outbound counters and memory observations are available; traffic/memory histories default on. Accepted configuration sources, validation and reload operations require a captured `.dae` startup; content and writes require separate opt-ins. Group mutations, provider/geodata management, probes, runtime settings/mode/suspend/resume and connection closing remain unavailable. Discover the actual enabled capabilities rather than assuming every resource is present.
...
| GET | `/api/v1/operations/{id}` | Actual queued/running/succeeded/failed reload state and safe result/error. |
...
Health rows are completed, qualified producer observations—not optimistic alive flags, copied IPv4/IPv6 ranking signals, synthetic failures or restored delays. Raw TCP, HTTP response headers, DNS exchanges and QUIC handshakes retain actual destination family and completion time. Unknown averages/ranking/warmth remain null/unknown. Group-specific samples preserve the measured member and leaf rather than rebinding to a later selection. GET never advances URLTest, round-robin or Score state. Icons are null until configured support exists.

All group writes remain explicitly disabled by the M3b gate, even though M6 source writes are available. Per-network Selector mutation still needs the shared control/reload/persistence/warm-owner cutover, and JSON Patch needs integration with the source transaction. No memory-only PATCH or synthetic interruption is provided; automatic override also remains gated by the unresolved combined-network response contract.
...
Published events are `stream.ready`, `runtime.updated`, `flow.updated`, `flow.gap` and `generation.changed`, plus `operation.updated` when the configuration coordinator is available. Operation events reflect real admitted reload transitions; generation events originate only at accepted publication, not reload receipt. Events contain bounded safe IDs/state, not packet bodies or raw configuration. Flow/event retention is memory-only, not a durable log.
