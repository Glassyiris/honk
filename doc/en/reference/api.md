...
| `blockers` | Fixed counts: `recovery`, `backoff`, `qualification`, `availability`, `responseMissing`, `responseUnpaired`, `responseMisaligned`, `probeScope`, `responseDegraded`, `nodeFailure`, `targetFailure`. Reasons can overlap and are not additional failures or counters. Excluded candidates are counted separately; node/target failure counts include them. No target keys are exposed. |
