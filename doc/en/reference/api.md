...
For accepted `.dae` rules in non-credential sources, `expression` retains the authored condition values (including geosite/geoip names, negation and quoted arguments), with comments and the outbound clause removed. This rule metadata does not require `config_content` or write permission; it neither reveals full source content nor expands geodata. Credential-bearing sources and rules without accepted source metadata retain redacted compiled summaries. Disk edits do not change the dictionary until a reload is accepted; rejected reloads retain the previous expressions. Trace and historical-flow expression redaction is unchanged.
