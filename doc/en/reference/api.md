...
Comparison labels describe empirical evidence, not whether the ordinary switching margin was crossed. Completion maturity alone cannot retract an unchanged qualified response relation; a held but unsupported member or measured tradeoff may remain `unconfirmed` with no missing response samples. Unknown or discarded directions do not enter response-cohort identity. A qualified pairwise rival advantage still prevents false support even when another member lacks that direction. Incompatible qualified response cohorts instead set a response gap and request budgeted comparable traffic for the control and participating challengers; this does not create work on reads or guarantee completion. Direction-known flags require coherent directional coverage, and validity follows all consulted qualified support.
