...
| `coverage` | `scope` (`all`, or `bounded` when the group exceeds its demand-sized evaluation set), `candidates`, `evaluated`, `unevaluated`, `compared`, `pending` and `excluded` counts. A bounded claim covers only evaluated members; `unevaluated` members are neither compared, excluded nor beaten, and `evaluated` includes one rotation member that joins the claim only with evidence. The remaining counts are within the evaluated set, plus `targetLimited` when common-target support is partial (unqualified matched targets or more than the eight canonical ones). Pending includes qualification/recovery work even with availability/response support. `excluded` counts members resolved without a pair: a recent failure exclusion, or an ineligible member whose observed reliability is below the selection's while both have four effective completions. Such a member keeps its recovery/validation work, so it can also be pending, and its unpaired metrics are not called equivalent. A singleton can be usable without beating another path. |
| `blockers` | Fixed counts: `recovery`, `backoff`, `qualification`, `availability`, `responseMissing`, `responseUnpaired`, `responseMisaligned`, `probeScope`, `responseDegraded`, `nodeFailure`, `targetFailure`. Reasons can overlap and are not additional failures or counters; `qualification` counts every unresolved unqualified member once any member is qualified, even when `question` names another gap. Excluded candidates are counted separately; node/target failure counts include them. No target keys are exposed. |
