...
| `localComparison` | Bounded active-challenger summary below; it never upgrades incomplete whole-candidate coverage to a global comparison. |
| `coverage` | `candidates`, `compared`, `pending` and `excluded` counts, plus `candidateLimited` and `targetLimited` booleans for incomplete bounded coverage. Pending includes qualification/recovery work even with availability/response support. A singleton can be usable without beating another path. |
