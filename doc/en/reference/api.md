...
The frozen contract is [api-standardize cb8ac07c6520b7fb08539cc0b7701695f5a07992](https://github.com/Zakkaus/api-standardize/tree/cb8ac07c6520b7fb08539cc0b7701695f5a07992). Its `base` profile is implemented, not `full_transparency`. Observations, bounded diagnostics, Selector control, connection closing and reversible datapath suspension are available as described below. Source administration requires a captured `.dae` startup, with separate content/write opt-ins. Automatic-policy overrides, node/provider CRUD and geodata management (M9), and embedded/real doona integration remain unavailable. Provider reads and refresh are not CRUD. Discover current capabilities rather than assuming every action is enabled.
...
| GET | `/api/v1/flows`, `/api/v1/flows/{flow_id}` | Active and retained terminal userspace decisions; detail includes the captured partial trace. |
...
| GET | `/api/v1/config`, `/api/v1/config/sources/{source_id}` | Accepted source metadata and safe diagnostics; exact content only when authorized and enabled. |
...
Recording defaults on with the native listener and does not depend on dashboard subscriptions. `record_flows: false` disables recording and releases its buffers on restart. One process-local store retains at most 1024 flows, 64 steps per flow and 8 MiB including snapshots; terminal records expire after at most 300 seconds, earlier under pressure. Restart clears all records. The existing sampler performs expiry without a second timer.

Flow IDs represent incarnations, never tuples. TCP and UDP capture actual route/sniff/verification/selected-leaf/attempt and terminal boundaries; failed or blocked setup is retained even without a live connection. Names, IDs and generations are captured at the decision, not joined to today's group selection. Kernel offload ends observation as unknown rather than inventing a closed connection. Rule internals, DNS substeps and lower-level transport attempts are not fully instrumented: traces remain partial, kernel/DNS scopes none, and `full_transparency` stays absent.

Flow list filters are `network`, `state`, `connection_id`, `detail`, `limit` and `cursor`. Eight bounded immutable snapshots live for at most 30 seconds; cursors bind instance and original filters/detail. Expired list snapshots return `410 snapshot_expired`, known retained tombstones `410 flow_expired`, unknown IDs `404 resource_not_found`; snapshot capacity returns 503 with Retry-After. Detail has no query parameters and always includes full retained input/trace. Counters are decimal strings; revisions, sequence and elapsed microseconds are safe JSON integers. Unsafe optional display fields are nulled without discarding causal IDs or outcomes.
...
The source manager requires a genuine `.dae` startup captured by the file loader. Programmatic configs and compatibility serde loaders do not provide lossless sources; their configuration/validation/reload capabilities remain unavailable. GET describes the **accepted snapshot**, not a fresh disk scan: opaque source IDs, exact-byte SHA-256 hashes, main/include kinds, write permissions and safe diagnostics. Private metadata paths are always `<redacted>`. Source hashes, configuration revision and runtime generation are distinct: accepting comments-only changes advances revision without requiring a new runtime generation; effective group membership changes revision, but health observations do not.

`config_content` and `config_write` default false and each requires a nonempty bearer secret. Exact source text is administrator-only, never anonymous. Entire API-credential-bearing sources are content-omitted and read-only, not returned with tokens replaced by saveable placeholders. Other disclosed source text is **not a sandboxed or generally redacted save payload** and can contain sensitive configuration. Do not save a metadata-only or redacted view as if it were the original. Native settings and API credentials cannot be changed or moved through this API; edit them locally and restart. To make a main file editable, first place API credentials in a dedicated read-only include locally.

With `config_write: true`, the accepted main source is writable unless credential-bearing. Includes require explicit `writable_includes` entries matching their canonical entry-directory-relative `.dae` paths; no absolute paths, traversal or glob authorization. Only accepted source IDs authorize replacement; caller-supplied paths do not authorize arbitrary file writes. Subscription/generated sources are not writable. Ordinary include glob ordering, entry-root confinement, duplicate/cycle rejection and no-match semantics remain unchanged; saving does not flatten or reformat other files.
