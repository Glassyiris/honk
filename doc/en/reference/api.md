...
| `coverage` | `scope` (`all`, or `bounded` when the group exceeds its demand-sized evaluation set), `candidates`, `evaluated`, `unevaluated`, `covered`, `compared`, `pending` and `excluded` counts. A claim covers only `covered` members: the selection plus ranked members admitted at their first qualification (every ranked member before any member qualifies); an admitted member stays until ranked out or reload, so its lapse reopens the claim. `unevaluated` members are neither compared, excluded nor beaten, and `evaluated` also includes one rotation member and ranked members still qualifying, which receive optional work and may veto but never block a claim. The remaining counts are within the evaluated set, plus `targetLimited` when common-target support is partial (unqualified matched targets or more than the eight canonical ones). Pending includes qualification/recovery work even with availability/response support. `excluded` counts members resolved without a pair: a recent failure exclusion, or an ineligible member whose observed reliability is below the selection's while both have four effective completions. Such a member keeps its recovery/validation work, so it can also be pending, and its unpaired metrics are not called equivalent. A singleton can be usable without beating another path. |
| `blockers` | Fixed counts: `recovery`, `backoff`, `qualification`, `availability`, `responseMissing`, `responseUnpaired`, `responseMisaligned`, `probeScope`, `responseDegraded`, `nodeFailure`, `targetFailure`. Reasons can overlap and are not additional failures or counters; `qualification` counts every unresolved unqualified covered member once any member is qualified, even when `question` names another gap. Excluded candidates are counted separately; node/target failure counts include them. No target keys are exposed. |
...
Comparison labels describe empirical evidence, not whether the switching margin was crossed. A held member or measured tradeoff can remain `unconfirmed` without missing samples. Qualified joint shared-block support can align different pair histories immediately; otherwise a response-alignment gap requests budgeted comparable control/challenger traffic. Original qualified pairwise rival advantages still veto support, even if outside the joint subset or another member lacks that direction. Direction-known flags require coherent coverage; the complete fingerprint and validity include all consulted qualified support. Reads never create work or guarantee completion.
