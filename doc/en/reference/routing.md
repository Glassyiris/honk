...
| `pname(...)` | Process name | `process_name` |
...
When health checking marks an outbound dead, the eBPF datapath drops new flows routed to it with `TC_ACT_SHOT`. It does not silently leak them through `direct`. Destination port `53` is exempt for both TCP and UDP so DNS can still reach the control plane.
