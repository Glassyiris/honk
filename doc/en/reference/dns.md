...
honk never overrides client-supplied ECS, including `/0`. Generated ECS is skipped for an upstream that declares `-> tag` or legacy `outbound:`; an upstream without one remains eligible even if ordinary traffic routing later selects a proxy. This static opt-out avoids pairing the direct-network prefix with an explicitly selected egress and disclosing it to that resolver. For an eligible named-upstream query without ECS, honk adds the configured option after cache/singleflight admission, validates any echoed ECS, and removes only its injected state before caching or replying. The effective prefix partitions DNS policy identity, so answers cannot cross a changed automatic prefix. `asis` requests remain byte-for-byte unchanged. ECS reveals an approximate client network to upstream authoritative servers; leave the field empty unless CDN locality requires it.
