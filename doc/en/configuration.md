...
**Cache file.** Set `experimental.cache_file.enabled: true` to persist Selector choices, delay samples and Clash mode in `<data_dir>/state/honk.db`; `store_dns: true` also persists eligible DNS answers. `path`, `cache_id` and `store_fakeip` no longer have an effect; the first start imports and removes a legacy `cache.db` ([upgrade notes](./reference/experimental.md#upgrading-from-cachedb)).
