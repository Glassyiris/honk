...
**Native API.** Build with `--features native-api`, then set `experimental.native_api.enabled: true` and a separate strong `secret`. The default address is `127.0.0.1:9527`; explicitly anonymous loopback is available for local development only. All native fields require restart. An optional `ui` directory must already contain readable `index.html`; no UI is downloaded. Traffic and memory histories default on (`record_traffic`, `record_memory`), retaining at most 600 points for 600 seconds without clients; disabling either releases its history buffer on restart. See the [native settings](./reference/experimental.md#native_api) and [native API](./reference/api.md#native-api-m1).

For a captured `.dae` startup, the native API exposes accepted source metadata, offline syntax/full validation and real reload operations. Exact administrator content and source writes are separate opt-ins (`config_content: false`, `config_write: false` by default), both requiring a nonempty bearer secret. The main file is writable when enabled unless it carries API credentials; accepted includes also need explicit canonical relative `writable_includes` entries (empty by default). Entire API-credential-bearing sources remain content-omitted/read-only, not redacted save templates. `.dae` stays authoritative: saves require a disk-content hash and operation tracking, and a rejected activation does not undo written bytes. Avoid concurrent manual edits during API saves; see [M6 safety and failure semantics](./reference/api.md#accepted-configuration-and-reload-operations-m6). Native group controls remain gated separately under M3b.
