...
- eBPF 从持久 pin 的 `UDP_DECISION_SEQUENCE` 分配非零 token；token 由两位 generation 和 28 位 sequence 组成。它发布 token 绑定的 handoff/redirect/`ConnState::Pending`，再把 skb 标为 Pending。唯一的 raw-netlink listener 向同时受 256 项和 8 MiB payload 上限约束的 ingest actor 投递。backend 锁等待与 Arm 前的每次转换都沿用从 listener 收包开始计算的固定三秒绝对期限；饱和或超期均 fail closed 丢包。独立的一秒 sampler 读取内核队列状态并刷新 actor 最老年龄，因此 dispatcher 停顿不会掩盖压力。队列 `320` 在 conntrack/NAT 之前保留原始 skb；没有 bypass、fanout 或 fail-open。
...
- 重载与关闭先发布 NFQUEUE-not-ready，切换双槽 decision epoch，等待所有 fence 前的 per-CPU reader，并删除残留 Preparing/Pending state。延迟队列包因此无法跨 runtime generation。精确回收另行安装 `BPF_NOEXIST` 五元组 fence，等待 fence 前 reader，重新校验并删除。sequence 耗尽时使用同一 fence/drain 协议，完整扫描会跨越成功但不足一批的 BPF batch，直到终止 `ENOENT`，再选择 conn-state、handoff、redirect 与 retirement-fence map 均未使用的 generation。若四个都仍存活，则按 1、2、5、30 秒退避重试，暂存保持 fenced；无需暂存的 UDP 继续工作而新的歧义流 fail closed。加载时会迁移持久化的旧线性 allocator 值且不复用 token。队列、listener、verdict 和回收故障仍为致命错误，但耗尽无需重启系统。
