...
- eBPF 从持久 pin 的 `UDP_DECISION_SEQUENCE` 分配非零 token；token 由两位 generation 和 28 位 sequence 组成。它发布 token 绑定的 handoff/redirect/`ConnState::Pending`，再把 skb 标为 Pending。唯一的 raw-netlink listener 向一个有界 ingest actor 投递；actor 会等待权威 backend read，不会因锁竞争丢包。队列 `320` 在 conntrack/NAT 之前保留原始 skb；没有 bypass、fanout 或 fail-open。
...
- 重载与关闭先发布 NFQUEUE-not-ready，切换双槽 decision epoch，等待所有 fence 前的 per-CPU reader，并删除残留 Preparing/Pending state。延迟队列包因此无法跨 runtime generation。精确回收另行安装 `BPF_NOEXIST` 五元组 fence，等待 fence 前 reader，重新校验并删除。sequence 耗尽时使用同一 fence/drain 协议，选择所有存活 token 绑定 map 均未使用的 generation；若四个都仍存活，暂存保持 fenced 并持续重试，无需暂存的 UDP 继续工作而新的歧义流 fail closed。队列、listener、verdict 和回收故障仍为致命错误，但分配器耗尽无需重启系统。
