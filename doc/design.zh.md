...
- （**实验特性，默认关闭**——`experimental.udp_nfqueue`，第一阶段骨架。）原先的"丢 Initial 等重传"（drop-and-PTO）决策后卸载已移除。取而代之的是**暂存决策**路径：Rule 模式下非 `must`、且无法路由时卸载的 direct UDP 流，在 `lan_ingress` 里发布 handoff、把 conn_state 置为 `Pending` 并写入随机 `decision_cookie`、给 skb 打上 `NFQUEUE_PENDING_MARK`（第 29 位）后放行进入协议栈，由 `honk_nfqueue` inet prerouting 链（优先级 -250，在 defrag 之后、conntrack 之前）把这些包排入用户态 worker（`crates/honk-nfqueue`，手写 NETLINK_NETFILTER，不依赖 `nft` 命令）。第一阶段的 worker 通过 `EbpfBackend::offload_udp_flow` 提交 `DirectActive`，并对原 skb 发 NF_ACCEPT（清掉 pending 位）——首个数据报端到端保持客户端自己的五元组（不经用户态 socket、无 PTO），后续包按卸载位直接内核直通。仍处于 `Pending` 的流的后续包会继续进 NFQUEUE，直到提交完成，因此暂存流绝不会在决策中途被切到 TPROXY 路径。每个入队包由 RAII guard 保证恰好一次 verdict（未提交的 guard 一律 NF_DROP 收尾）；规则集原子安装，且只删除 honk 自己创建并记录了 handle 的对象。流水线在数据面准入开放之前完成启动（listener → 规则 → 自检 → `DATAPATH_FLAG_NFQ_READY`），任何一步失败都拒绝启动；特性关闭时数据面与未引入 NFQUEUE 前逐字节一致。收不齐的分片 ClientHello 仍按未决丢弃等待重传（sniffer 会话保留分片），绝不按纯 IP 猜测决策；proxy/block 提交路径、pending 流池与 QUIC 驱动的暂存决策在下一阶段落地。
