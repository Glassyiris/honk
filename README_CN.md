...
修改 `global.nfqueue_enable` 后必须重启。该路径必须使用带 `ebpf` feature 的构建和真实 eBPF 后端；`--mock-ebpf` 或不带 `ebpf` 的构建会在启动时被拒绝。本机发起的 WAN 出口流量仍走规范 TPROXY 路径。DNS 53、`must`、`block` 和已经可以安全地在路由时直连的决策不会进入 NFQUEUE；只暂存仍可能在用户态改判的决策。
...
高层路径：通常为 **TC 分类 → 经 `dae0`/`daens` redirect → sk_lookup 透明监听 → 用户态拨号/中继**；`global.nfqueue_enable` 开启时（默认开启），仍有歧义的 LAN 转发 UDP 会改为 **TC 暂存 → NFQUEUE 保留原始 skb → token 校验后的 direct/proxy/block 提交**。细节见设计文档。
