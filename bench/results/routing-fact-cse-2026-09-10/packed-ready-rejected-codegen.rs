...
fn fact_caches(plan: &RoutingPushPlan) -> [Option<FactCache>; 3] {
    ...
    // Pointers occupy [-24, -1], flags [-36, -25]; keys start at -64 and -96.
    std::array::from_fn(|index| {
        (uses[index] == 2).then_some(FactCache {
            pointer: -8 * (index as i16 + 1),
            ready: -28 - 4 * index as i16,
        })
    })
}
...
pub fn emit_routing_program(
    ...
    for cache in caches.iter().flatten() {
        asm.emit(BPF_ST | BPF_DW | BPF_MEM, R10, 0, cache.pointer, 0)?;
        asm.st_imm(R10, cache.ready, 0)?;
    }
    ...
    match predicate {
        ...
        KernelPredicate::DestinationIp(id) => {
            emit_fact_bit(
                ...
                *id,
                caches[FactKind::Destination as usize],
                on_true,
                on_false,
                fds,
            )?;
        ...
        KernelPredicate::SourceIp(id) => {
            emit_fact_bit(
                ...
                FactKind::Source,
                *id,
                caches[FactKind::Source as usize],
                on_true,
                on_false,
                fds,
            )?;
