...
BASELINE_COMMIT = "d2cf18438debba60d87ce46ba6da68bca6f072b3"
BASELINE_SHA256 = "20b09db070e0da3344e17d6a32b6de50023fd4d7d7725af9e7e9add79889efa9"
BASELINE_METADATA_SHA256 = "670f0c6ed0a6ec808c1bbfb5a1d901e40e5a9ddf332b85fdcfce4060dfe8808e"
...
def load_program(body, actual_fds, wrapper, label):
    ...
    metadata = {
        ...
        "verifier": {
            "processed_instructions": int(summary.group(1)) if summary else None,
            "max_states_per_instruction": int(summary.group(2)) if summary else None,
            "total_states": int(summary.group(3)) if summary else None,
            "peak_states": int(summary.group(4)) if summary else None,
            "stack_depth_bytes": int(stack.group(1)) if stack else None,
            "stack_depth_note": None
            if stack
            else "kernel verifier summary did not report stack depth; per-path interpreter depth is recorded",
        },
    }
...
def routing_input(family, index=1, *, miss=False, mac_present=True, destination_port=443):
    value = bytearray(128)
...
def configure_case(case, kernel_maps, sentinels):
    ...
    input_value = routing_input(
        ...
        destination_port=8443 if scenario == "early-port" else 443,
    )
    if case["variant"].startswith("dst-"):
        count = int(case["variant"].split("-")[1])
        if count and scenario in ("first", "middle", "last", "multi"):
...
def correctness_cases():
    ...
    for count in (1, 4, 16, 64, 256):
        for family in (1, 2):
            for scenario in ("first", "middle", "last", "zero", "miss"):
                cases.append({"variant": f"dst-{count}", "family": family, "scenario": scenario})
    cases.extend(
        [
            {"variant": "dst-256", "family": family, "scenario": "multi"}
            for family in (1, 2)
        ]
    )
    ...
    cases.append({"variant": "mixed", "family": 0, "scenario": "invalid"})
    return cases
...
def benchmark_input(kernel_maps, family, scenario, count):
    kernel_maps.clear()
    input_value = routing_input(family, miss=scenario == "miss")
    if count and scenario != "miss":
        selected = 0 if scenario == "first" else count // 2 if scenario == "middle" else count - 1
        value = bytes(32) if scenario == "zero" else bitmap(selected)
        kernel_maps.put("destination", family, fact_key("destination", family, input_value), value)
    return input_value
...
def run(args):
    ...
    for family in (1, 2):
        for count in (16, 256):
            for scenario in ("last", "zero", "miss"):
                input_value = routing_input(family, miss=scenario == "miss")
                if scenario != "miss":
                    large.put(
                        "destination",
