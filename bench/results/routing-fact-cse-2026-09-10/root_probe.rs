...
fn paired_root_slot() {
    ...
    for count in [0, 1, 4, 16, 64, 256] {
        for early in [false, true] {
            ...
            for family in [4, 6] {
                for position in ["first", "middle", "last", "miss"] {
                    let index = match position { "first" => 0, "middle" => count / 2, "last" => count.saturating_sub(1), _ => 256 };
...
fn mixed_extension_statistics() {
    ...
    for count in [1, 4, 16, 64, 256] {
        ...
        for candidate in [false, true] {
            ...
            let mut row = serde_json::json!({"count_per_category":count,"candidate":candidate,"emitted_slots":code.insns.len(),"load_us":load_us,"verifier":verifier_text(&log)});
            match loaded {
                Ok(program) => {
    ...
    std::fs::write(std::env::var("HONK_CSE_EXT_RESULTS").unwrap(), serde_json::to_vec_pretty(&rows).unwrap()).unwrap();
