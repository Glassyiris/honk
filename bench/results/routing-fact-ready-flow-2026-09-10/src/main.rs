...
const BASELINE_COMMIT: &str = "ac7cf6a5ab6d7c4229a0c6230a6415965d953403";
const BASELINE_SOURCE_SHA256: &str =
    "477f81f42c422783d5a2790b6d98e34372e23a4c51a1777357dfbf1497543d3a";
const BASELINE_METADATA_SHA256: &str =
    "9ff8a368f80f4d5dec99837a489d58e8943234fbf15fa64629c5194971bb09bc";
...
fn dominated_destination_chain_plan() -> RoutingPushPlan {
    plan(
        (0..64)
            .map(|index| {
                rule(
...
fn domain_gated_repeated_fact_plan() -> RoutingPushPlan {
    let mut result = plan(
        (0..64)
            .map(|index| {
                rule(
    ...
    result.features = 3;
    result.has_domain_rules = true;
    result.domain_predicate_count = 1;
    result
}

fn mixed3categories_plan() -> RoutingPushPlan {
    plan(
        (0..64)
            .map(|index| {
                rule(
...
fn conditional_first_use_merge_plan() -> RoutingPushPlan {
    plan(vec![
        rule(
            13_000,
            vec![
        ...
        rule(13_001, vec![condition(KernelPredicate::DestinationIp(1))]),
        rule(
            13_002,
            vec![
                condition(KernelPredicate::DestinationIp(2)),
                condition(KernelPredicate::DestinationPort(vec![
                    routing::PortRange { start: 9, end: 9 },
                ])),
            ],
        ),
        rule(13_003, vec![condition(KernelPredicate::DestinationIp(3))]),
    ])
}

fn seeded_adversarial_plan() -> RoutingPushPlan {
    const SEED: u32 = 0x484f_4e4b;
    let mut state = SEED;
    ...
    for index in 0..32 {
        ...
        let id = (state >> 27) & 31;
        let destination = KernelPredicate::DestinationIp(id);
        let source = KernelPredicate::SourceIp((state >> 21) & 31);
        let mac = KernelPredicate::Mac((state >> 15) & 31);
        let first = match state % 3 {
            0 => destination,
            1 => source,
            _ => mac,
        };
        let second = if state & 4 == 0 {
            KernelPredicate::Protocol(2)
        } else {
            KernelPredicate::DestinationPort(vec![routing::PortRange {
                start: 80,
                end: 8443,
            }])
        };
        let conditions = if state & 1 == 0 {
            vec![condition(first), condition(second)]
        } else {
            vec![not(first), condition(second)]
        };
        rules.push(rule(14_000 + index, conditions));
    }
    plan(rules)
}
...
fn variants() -> Vec<Variant> {
    ...
    for (name, plan) in [
        ("dominated-dst-chain", dominated_destination_chain_plan()),
        ("domain-gated-repeated", domain_gated_repeated_fact_plan()),
        ("mixed3categories", mixed3categories_plan()),
        ("conditional-first-use-merge", conditional_first_use_merge_plan()),
        (
            "seeded-adversarial-0x484f4e4b",
            seeded_adversarial_plan(),
        ),
    ] {
        variants.push(Variant { name, plan });
    }
    variants
}
...
fn main() -> anyhow::Result<()> {
    let output = PathBuf::from(
        std::env::args_os()
            .nth(1)
            .context("usage: honk-routing-fact-ready-flow-evidence OUTPUT_DIRECTORY")?,
    );
    ...
    let candidate_source = Path::new(env!("HONK_CODEGEN_PATH"));
        ...
        sha256_bytes(include_bytes!("../baseline-codegen.rs"))? == BASELINE_SOURCE_SHA256,
        "compiled baseline emitter differs from frozen baseline"
    );
    ensure!(
    ...
    let candidate_source_sha256 =
        sha256_bytes(include_bytes!(env!("HONK_CODEGEN_PATH")))?;
    ensure!(
        sha256(candidate_source)? == candidate_source_sha256,
        "candidate source changed since generator compilation"
    );
    ...
    for variant in variants() {
        ...
        manifest_variants.push(json!({
            ...
            "emitters": {
                "old": {
                    "file": old_name,
                    "bytecode_sha256": sha256(&output.join(&old_name))?,
                    "instruction_slots": old.len(),
                    "static_bytes": std::mem::size_of_val(old.as_slice()),
                    "map_lookup_call_sites": old.iter().filter(|insn| insn.code == 0x85 && insn.imm == 1).count()
                },
                "new": {
                    "file": new_name,
                    "bytecode_sha256": sha256(&output.join(&new_name))?,
                    "instruction_slots": new.len(),
                    "static_bytes": std::mem::size_of_val(new.as_slice()),
    ...
    let manifest = json!({
        ...
        "metric_definitions": {
            ...
            "map_lookup_call_sites": "static helper call-site count",
            "kernel_processed_instructions": "recorded by the verifier log during load",
            "jit_bytes": "recorded from BPF program info during load",
            "stack_depth_bytes": "recorded from the verifier log when present",
        },
        "seeded_adversarial_seed": 0x484f_4e4b_u32,
        "baseline": {
