...
payload limits. Backend-lock waits retain each packet's absolute three-second
deadline from listener receipt; saturation or expiry drops fail closed. The crate
...
- `src/stats.rs` — `StatsManager` owns the fixed allocation-free `GET /stats` UDP schema. `udp.nfqueue` includes listener/correlator counts, actor depth/queued bytes/oldest age, kernel queue depth/drop counters plus explicit latest-read availability and cumulative read errors, held/peak guard gauges, effective receive-buffer size, terminal verdicts, token exhaustion/rotation, verdict errors, and `receiptToVerdict`. The latency is listener receipt-to-successful-verdict, not kernel queue residence.
...
| `test-netns` | Root-gated real-kernel tests: production NFQUEUE/nftables IPv4+IPv6 held-verdict contract, eBPF netlink/netns roundtrips, link ownership/rebind lifecycle, and legacy pinned allocator migration (`--features ebpf --ignored`, serial) |
...
`.github/workflows/ci.yml` runs fmt + clippy, the workspace gate, and an Ubuntu hosted-VM eBPF job. The VM job BTF-checks the kernel object, mounts bpffs, runs the real NFQUEUE/nftables netns contract, TC/cgroup link lifecycle, legacy pinned allocator migration, and root-only network tests serially without a job container.
...
- **Cleanup:** graceful shutdown fences NFQUEUE producers/guards and deletes the exact owned nftables table last. `just clean-all` removes `dae0`/`daens`, ephemeral BPF pins, and policy routes but deliberately preserves `UDP_DECISION_SEQUENCE`; never manually reset or delete the allocator to recover exhaustion—the fenced supervisor rotates generations and backs off while all four remain live. **Never start a second real datapath instance:** `/run/honk-core.lock` prevents overlap over fixed names.
