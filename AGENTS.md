...
- **Cleanup:** graceful shutdown fences NFQUEUE producers/guards and deletes the exact owned nftables table last. `just clean-all` removes `dae0`/`daens`, ephemeral BPF pins, and policy routes but deliberately preserves the rollback-compatible `UDP_DECISION_SEQUENCE`; never reset or delete it to recover normal exhaustion—the fenced supervisor rotates generations and backs off while all four remain live. If startup rejects a corrupt or incompatible pin, keep staging fenced, stop every honk process, verify the queue and token-bound maps are gone, then remove the pin once before restarting. **Never start a second real datapath instance:** `/run/honk-core.lock` prevents overlap over fixed names.
