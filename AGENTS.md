...
- DNS hosts snapshots are generation-pinned: `use_host` loads `/etc/hosts` or the OxiDNS-compatible `hosts_file` before request routing, cache lookup, and upstream exchange. Relative custom files resolve through `global.data_dir`; SIGHUP replaces the immutable snapshot transactionally, and a load failure leaves the active generation untouched.
...
- DNS hosts snapshot: `dns.use_host` defaults false. Empty `hosts_file` reads standard `/etc/hosts`; a non-empty file uses OxiDNS-compatible exact/domain/regexp/keyword rules. Relative paths resolve below `global.data_dir` with legacy working-directory fallback. Known IN A/AAAA names precede request rules and cache; a missing family returns NODATA without upstream I/O. SIGHUP reloads file contents transactionally.
