...
- VLESS Encryption reuses `Node.encryption`: `Node::derive_id()` treats a non-`none` VLESS value as credential identity, while legacy plain VLESS IDs remain unchanged. `Config::validate` rejects combining it with `flow`.
- `src/dns.rs` — much richer than a plain upstream list: `DnsConfig` (`bind`, `upstream`, `routing`, `strategy`, `cache`, `fixed_domain_ttl`); `DnsBindEndpoint` / `DnsBindTransport` / `DnsBindError` implement current-dae standalone-listener parsing (empty maps to disabled through `DnsConfig::bind_endpoint`; bare numeric `IP:port` is UDP; schemes are `udp://`, `tcp://`, `tcp+udp://`, with explicit port and semantic equality); `DnsUpstream` (name, address, `protocol: DnsProtocol`, `tls_server_name`, **`outbound: Option<String>`** — per-upstream dial-path proxy tag); and the dae-shaped DNS routing model (`DnsRequestRule`/`DnsResponseRule` with AND-ed `DnsCond`s — Qname/Qtype/Upstream/Ip, each negatable — first match wins; actions Reject/AsIs/Accept/Upstream(name); legacy `rules`/`fallback` with conversion). `types.rs::DnsProtocol` has 6 variants: Udp, Tcp, Tls (DoT), Https (DoH), H3 (DoH3), Quic (DoQ). Request/response routing types are populated only by the dae parser (deliberately outside the serde tree).
...
- VLESS share links map `encryption=` into the same field; Clash YAML subscriptions map `encryption` (with `cipher` fallback) before deriving the node ID.
...
non-Clone exactly-once `VerdictGuard` whose drop verdict is `NF_DROP`. It owns an
atomic nftables transaction for exact `inet honk_nfqueue` / `udp_decision` at
prerouting priority `-250`, selected only by the pending mark; the queue rule has
no bypass, fanout, or fail-open. Malformed/truncated input, ENOBUFS, listener exit,
or verdict-socket failure is fatal. Startup binds the queue before publishing the
owned rule; explicit shutdown requires producers/guards fenced before closing the
queue and deleting the table. Same-netns firewall managers must not mutate the
owned names while honk runs.
...
- `src/session.rs` — **unified node-owned session pool v2** for multiplexed outbounds (AnyTLS; QUIC protocols keep the single-connection `quic::QuicClient` holder): one pool per node runtime with a hard session cap and least-loaded scheduling over `Active` sessions (`SessionState` Active/Draining/Closed); Draining sessions take no new channels. **Pool-owned dial tasks** mean the first caller registers and the pool spawns the dial; cancelling a caller ends only its wait, never the shared dial (`DialSignal` broadcasts outcomes; `DialGuard` is the panic backstop). Cold-URLTest AnyTLS is deliberately different: a caller-owned provisional slot counts against the same hard cap, loser cancellation drops its physical dial, and only the finalized winner publishes into the generation pool. The pool also provides exponential dial-failure backoff, one node janitor (min-idle prewarm, idle reaping, jittered max-age drain), metrics, graceful retirement (reject new work, drain live sessions), and idempotent force shutdown (wake waiters, close sessions, zero tasks). Capacity is a per-session **semaphore permit** (`try_reserve` = Active→acquire→recheck; `open_with` does atomic reserve+open, retrying once on a fresh session for `OpenError::Session`, never for `Refused`). Every admitted session remains tracked even during a transient over-cap race; otherwise its demux task could hold an orphaned connection forever.
  AnyTLS enables idle-first spreading across its two physical sessions before least-loaded reuse, preventing one lossy TCP connection from serializing every UoT stream.
...
- `src/dns/` — generation-pinned userspace DNS split by invariant:
  ...
  - `forwarder/`, `engine/`, `planner/`, and `policy.rs` implement parse → strategy → request routing → exact-identity cache/singleflight → upstream → bounded response re-query. Production `DnsService` callers use strict query/response wire validation before cache publication; the raw `DnsForwarder::resolve*` compatibility surface remains for legacy internal callers. `asis` preserves ingress transport, including UDP TC→TCP fallback; prefer-family sibling queries preserve the caller wire profile except QTYPE.
  - `cache/` is a sharded LRU bounded by both `max_cache_size` entries and retained query/response wire bytes (4 KiB per configured entry, at least 65,535 bytes per shard, globally capped at 64 MiB), with negative TTL clamping, one-hour serve-stale, stale-while-revalidate, and `HDNS` v2 persistence keyed by canonical wire, ingress profile, logical scope, policy, and operation. An insertion evicted immediately by the wire-byte budget is not persisted.
...
- `src/ebpf/` — `EbpfBackend` exposes token/state inspection plus `commit_udp_decision(key, token, transition)`, token-checked abort/removal, kernel staging quiescence, persistent allocator validation, locked exhaustion reads, and ordinary routing/map operations. `ArmDirect` requires Pending and retains the token; marked verdicts run before `ActivateDirect`; `ActivateProxy` publishes final outbound/mark while retaining its provisional track; Block/abort clean only the matching incarnation. Real retirement installs a `BPF_NOEXIST` tuple fence, flips the reader epoch, waits for the prior slot, then revalidates token/state/auxiliaries before deletion; it never overwrites `CONN_STATE_MAP` with a cached value. Real and mock backends implement identical Missing/TokenMismatch/StateMismatch outcomes; ordinary real-backend cleanup preserves `UDP_DECISION_SEQUENCE`.
...
- `src/stats.rs` — `StatsManager` owns the fixed allocation-free `GET /stats` UDP schema, including `udp.nfqueue` (`received`, `activeFlows`, `directAccepted`, `proxyCopied`, `proxyDropped`, `block`, `cancel`, `drop`, `tokenMismatch`, `tokenExhaustion`, `verdictErrors`, `receiptToVerdict`) plus existing endpoint/warm metrics. `receiptToVerdict` is listener receipt-to-successful-verdict latency, not kernel queue residence.
- `src/clash_api.rs` + `clash_api/{logs,doh,ui}.rs` — Clash-compatible REST/WS API and external UI. `GET /stats` returns the userspace snapshot (not eBPF `OUTBOUND_STATS`). Mode and GLOBAL selection mutations go through `DatapathFlagsHandle`, so reload fencing and the latest mode/static bits compose atomically; selector mutations remain group-manager operations.
...
6. DNS: transparent port-53 flows and optional host-netns `dns.bind` UDP/TCP queries share `DnsController` → singleflight → `DnsForwarder` (request routing → cache → upstream pool: UDP/TCP/DoT/DoH/DoQ/DoH3, optionally through a proxy leaf → response routing) → answers are pushed into `DOMAIN_ROUTING_MAP` so eBPF can route subsequent connections to those IPs. The standalone adapter uses `original_dst=None`; the transparent adapter preserves the intercepted destination.
...
| `test-netns` | Root-gated real-kernel tests: production NFQUEUE/nftables IPv4+IPv6 held-verdict contract, then eBPF netlink/netns roundtrips and `link_lifecycle` (`--features ebpf --ignored`, serial) |
...
`.github/workflows/ci.yml` runs fmt + clippy, the workspace gate, and an Ubuntu hosted-VM eBPF job. The VM job BTF-checks the kernel object, mounts bpffs, runs the real NFQUEUE/nftables netns contract plus TC/cgroup `link_lifecycle`, and executes root-only tests serially without a job container.
...
- Test locations:
  ...
  - `crates/honk-outbound/src/reality.rs` + `proxy/vless.rs` + `proxy/vless_encryption.rs` (`#[cfg(feature = "rprx")]`) — REALITY session_id/authKey vectors; VLESS header layouts, lazy response-header stripping, and Vision unpadding; VLESS Encryption byte-context BLAKE3 KDF, padding/config parsing, AEAD frame/backpressure contracts, plus an ignored live Xray 1-RTT→0-RTT interoperability check.
...
- **Cleanup:** graceful shutdown fences NFQUEUE producers/guards and deletes the exact owned nftables table last. `just clean-all` removes `dae0`/`daens`, ephemeral BPF pins, and policy routes but deliberately preserves `UDP_DECISION_SEQUENCE`; never reset/delete that allocator to recover exhaustion—reboot. **Never start a second real datapath instance:** `/run/honk-core.lock` prevents overlap over fixed names.
