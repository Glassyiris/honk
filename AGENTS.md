...
ingest actor serializes listener deliveries and waits for backend reads. The crate
owns an atomic nftables transaction for exact `inet honk_nfqueue` / `udp_decision`
at prerouting priority `-250`, selected only by the pending mark; the queue rule
has no bypass, fanout, or fail-open. Malformed/truncated input, ENOBUFS, listener
exit, or verdict-socket failure is fatal. Startup binds the queue before publishing
the owned rule; explicit shutdown requires producers/guards fenced before closing
the queue and deleting the table. `QueueStats` reports kernel queue depth/drops,
held guards, and the effective socket receive buffer. Same-netns firewall managers
must not mutate the owned names while honk runs.
...
- `src/control/` — the control plane:
  - `mod.rs` — `ControlPlane`: TPROXY TCP/UDP listeners plus the supervised NFQUEUE runtime, one bounded ingest actor, fixed startup descriptor budgets, listener-FD publication, queue-pressure sampling, periodic locked token-exhaustion detection, safe generation rotation, and ordered startup/shutdown. Queue/listener/verdict faults are fatal branches of `run`; sequence exhaustion fences/drains and rotates to a generation absent from live token-bound maps.
...
- `src/ebpf/` — `EbpfBackend` exposes token/state inspection plus `commit_udp_decision(key, token, transition)`, token-checked abort/removal, kernel staging quiescence, persistent allocator validation/status/reset, and ordinary routing/map operations. Tokens combine a two-bit generation with a 28-bit sequence. A reset is allowed only after staging is fenced/drained and only to a generation absent from live conn-state/handoff/redirect maps. `ArmDirect` requires Pending and retains the token; marked verdicts run before `ActivateDirect`; `ActivateProxy` publishes final outbound/mark while retaining its provisional track; Block/abort clean only the matching incarnation. Real retirement installs a `BPF_NOEXIST` tuple fence, flips the reader epoch, waits for the prior slot, then revalidates token/state/auxiliaries before deletion; it never overwrites `CONN_STATE_MAP` with a cached value.
...
- `src/stats.rs` — `StatsManager` owns the fixed allocation-free `GET /stats` UDP schema. `udp.nfqueue` includes listener/correlator counts, kernel queue depth and drop counters, held/peak guard gauges, effective receive-buffer size, actor saturation, terminal verdicts, token exhaustion/rotation, verdict errors, and `receiptToVerdict`. The latency is listener receipt-to-successful-verdict, not kernel queue residence.
...
- **NFQUEUE lifecycle is fatal/fenced:** listener, queue, verdict, watchdog, or retirement ambiguity terminates the control plane. Reload/shutdown clear ready, quiesce kernel stagers, reject new ingress, cancel/drain guards/leases/retirements, detach producers, close the queue, and delete the wholly owned table last. Ordinary cleanup preserves `UDP_DECISION_SEQUENCE`. Sequence exhaustion uses the same fence/drain, rotates to a generation absent from all live token-bound maps, and retries with staging fenced if all four generations remain live; it never resets into a collision and requires no reboot.
