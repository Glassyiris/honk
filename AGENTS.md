...
- **HTTP APIs:** axum 0.8 + tower-http 0.7. Default-on `clash-api` enables WebSockets; independent default-off `native-api` owns a Hyper HTTP/1.1 listener and needs no Clash/UI-download dependencies. `native_api.rs` owns state, routing, read handlers, sampler and supervisor; `native_api/{types,security,ui}.rs` own wire DTOs, HTTP boundaries and trusted-directory hosting. Native limits: 64 connections, 5s headers, 30s connection lifetime, 5s joined shutdown; one 1s sampler, no history buffers. Config/auth details: `.agents/rules/configuration.md` and `.agents/rules/security.md`.
