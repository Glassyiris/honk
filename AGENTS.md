...
- `src/stats.rs` (`StatsManager` per-outbound conns/bytes/errors plus the fixed `GET /stats` → `udp` endpoint/latency/capacity/slowPermit/queue/firstSend/stagger/warm schema), `src/pool.rs` (`ConnectionPool`: bare pre-handshake TCP 60s idle, ready dialed `ProxyStream` 30s idle, 8/key, 300s max age; budgets — 2048 global FD cap, 64 ready targets/node, hot-target gating (2 flows/60s) for speculative ready deposits, multiplexed handlers excluded at the deposit site; hit/miss/entry metrics on clash `/stats`; all entries of a node purged the moment it flips alive→dead via the alive-set death callback), `src/connection_tracker.rs` (feeds `/connections`; entries carry complete Clash metadata, matched rule/payload, leaf-first selection chain, monotonic traffic totals, and a lifecycle-owned close handle: TCP cancellation or generation-safe UDP pool removal), `src/mode.rs` (`ModeState` Rule/Global/Direct + GLOBAL selection; `override_outbound` never overrides block/must), `src/cachedb.rs` (rusqlite WAL: selector choices, clash mode, zashboard settings, `dns:` answers with lazy expiry, `delay:` last-real-latency samples per node (60s snapshot writer, restored at startup with 24h age-out — sing-box URLTest history storage parity), `cache_id` namespacing, corruption auto-reset to `*.corrupt-<ts>`), `src/subscription.rs` (fetch via reqwest: base64/simple, raw-line fallback, Clash YAML; share links via `Node::from_share_link`; `SubscriptionRefreshCoordinator` owns startup, immediate, manual, and periodic work, reconciles the exact active task set after acknowledged reload, joins it on shutdown, and sends identity-revalidated acknowledged merges through `ControlCommand::MergeSubscription`; **nodes live in memory only, never written back to the config file**).
- `src/clash_api.rs` + `clash_api/{logs,doh,ui}.rs` — capability-truthful Clash-compatible REST/WS API, started when `experimental.clash_api.external_controller` is non-empty. Auth: `Authorization: Bearer` or `?token=` (percent-decoded). `GET /version` advertises `sing-box honk <package-version>` so zashboard hides unsupported Mihomo actions. Endpoints: `GET /`, `GET /configs`, acknowledged `PUT /configs?reload=true` (DAE path only), mode-only `PATCH /configs`, proxies/selectors/delay, one row per DAE rule plus `Match`, REST/WS `/connections` with lifecycle-owned DELETE termination, shared WS/chunked `/traffic` and `/memory`, `GET /stats`, WS/chunked `/logs` (including truthful `silent`), `/dns/query`, cache flushes, enabled subscriptions via `/providers/proxies` plus acknowledged refresh and provider/node health checks, empty-compatible `/providers/rules`, `GET/PUT/DELETE /storage/zashboard`, `POST /upgrade/ui`, and `/ui` static hosting. Dashboard storage persists through `cachedb` when enabled. Startup download and POST update share UUID staging, index validation, and atomic `renameat2(RENAME_EXCHANGE)` publication; readers see only a complete old or new tree. Config replacement, restart/core/Geo upgrade, TUN, Smart, and rule-mutation routes remain absent. `logs.rs` is the tracing broadcast layer and skips formatting when no subscriber can consume the level.
...
- Test locations:
  ...
  - `crates/honk-core/tests/clash_api_test.rs` — Clash API endpoints and wire contracts (auth, capability profile, acknowledged config reload, proxies/delay/rules/providers, connection/traffic/memory/log streams, lifecycle close signaling, dashboard storage, DNS/cache, and UI hosting/update).
...
- Benchmarks: `cargo bench -p honk-core --features dns-bench --bench dns`; `cargo bench -p honk-core --bench udp -- --save-baseline udp-candidate`; and `bash bench/tests/udp-latency-cli.sh` for the `bench/udp-latency.sh` deployment-driver fixture. The candidate and live deployment-gate invocations are fixed in `doc/benchmark.en.md`.
