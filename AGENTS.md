...
- `src/session.rs` — **unified session pool** (P1.6) for multiplexed outbounds (h2mux, AnyTLS, Trojan-Go; QUIC protocols keep the single-connection `quic::QuicClient` holder): per-key pools with a hard session cap, least-loaded scheduling under a soft per-session stream cap, dial single-flight (watch-channel), exponential dial-failure backoff, per-key janitor (min_idle prewarm + idle reaping with per-node timeouts), pool metrics, shutdown. **`insert` always tracks, even over the cap** — an untracked session is orphaned from the janitor while its demux task holds the connection forever (beta.24 production incident: per-flow mux sessions leaked unboundedly on bare-TCP pool hits).
...
- `src/proxy/anytls.rs` — sing-anytls session multiplexing on the shared `SessionPool` (`host:port|pwhash|sni|verify` keys): demux task per session dispatching frames by `sid` (TCP sinks backpressure; **UoT sinks drop-on-full**), atomic sid allocator, FIN ends streams, per-key janitor pre-establishes `min_idle` sessions (default 1, so dials stay warm after the first) and reaps idle-expired ones; node fields `anytls_min_idle_session` / `anytls_idle_session_timeout` (the check interval is the pool-wide 30s janitor tick). UoT UDP uses the direct `open_uot_stream` path (see the invariant above), not the loopback bridge.
