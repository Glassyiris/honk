...
- **Allocator:** the shipped binary (`honk-core` bin) uses **mimalloc** as the global allocator behind the default-on `mimalloc` cargo feature (musl's stock malloc is slow under contention). mimalloc reserves aligned 1 GiB arenas and decommits purged pages, but fragment-pinned worker heaps linger. `main.rs` therefore builds Tokio explicitly, dispatches the top-level application future onto a worker, and runs `mi_collect(true)` from each owning worker's park callback with an OS-thread-local 60s cooldown and a delayed first collection. A bounded periodic rendezvous, attempted only when every other worker is idle, wakes owners that stayed parked after cross-thread frees so the same callback can purge their heap. `HONK_MI_COLLECT_SECS=0` installs neither the hook nor the rendezvous. RSS can still read as a traffic high-water mark under THP. The clash `/logs` tracing layer (`clash_api/logs.rs`) skips formatting entirely when it has no subscribers — it is unfiltered, so without the check every sub-level event in the data path would cost a `String`.
...
- `src/control/` — the control plane:
  ...
  - `connection.rs` — one canonical per-flow TCP/UDP route/sniff/mode/selection initializer. An authoritative single-candidate dial failure gets exactly one retry racing the URLTest latency-ordered top-3 (`urltest_retry_candidates`): the strike-moved pick is replaced, an unmoved pick re-races alongside its alternates so a lone transient failure (no strike until 2 consecutive) never hard-fails the flow; non-URLTest plans and single-leaf outcomes are not retried. Socket ingress and NFQUEUE-owned payloads enter the same `UdpInitLease`/`UdpEndpointPool`; `build_tuples_key` must stay `mem::zeroed()` because the kernel hashes all 40 bytes including padding. At the post-route boundary a staged flow computes one final outbound/mark and invokes token-checked NFQUEUE direct/proxy/block completion; there is no separate direct socket, cloned packet, packet replay, or deliberate retransmission path.
