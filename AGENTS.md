...
Observed availability uses four distinct targeted Traffic reporters with eligible RX after setup/TX per uninterrupted cell cohort, not terminal completions. Cell epochs and per-reporter/per-cell credits fence failure, reload, silence of at least 60 seconds and incarnation replacement; clones, repeated RX and settlement cannot multiply credits. Separate eligible availability RX from raw terminal RX used by ordinary recovery. Neutral settlement may flush factual eligible RX at its event time, never cleanup time or across expired support. Availability does not clear streaks or grant ordinary qualification: retain bounded qualification/recovery trials independently of availability/response gaps.
Shared-source replies may precede sender-task TX bookkeeping. Only the first successfully accepted positive send may reconcile that reporter's already delivered RX at or after its serialized send start and setup completion; capture start after source-gate waits, preserve actual RX time and all cell fences, and never turn queued/failed/retired sends or ordinary TX-only callbacks into receive evidence.
TCP copy/splice relay errors retain socket-side provenance and their original I/O cause. Only client-side disconnects (`ConnectionReset`, `ConnectionAborted`, `BrokenPipe`, `NotConnected`, `UnexpectedEof`, `TimedOut`) settle Score as cancellation: preserve factual RX and prior failures, never grant success or clear backoff. The same upstream errors, protocol errors, and unclassified local failures keep their existing failure treatment; connection-error telemetry remains unchanged. Never infer Score neutrality from log severity or error kind alone.

Score ordinary incumbent protection is separate from verification. Keep first-choice utility and existing trial budgets; an ineligible or unresolved-failure incumbent may escape without waiting. Recovery uses genuine targeted Traffic RX after setup/TX or a successful bidirectional terminal's actual RX, strictly newer than each applicable cell failure. Live publication is event-driven and capped at once per second per reporter; no timer guarantees delivery of throttled progress. Existing outcome counts, streaks/backoff and availability cohorts remain independent. Fresh business RX can retain already earned four-completion qualification for 60 seconds from RX time; below-threshold expired/invalidated leases cannot be revived by RX alone. Failure/reload invalidation and started-cell incarnation fences remain authoritative; neutral cancellation does not retract factual RX. Hold maturity and reliability blending still use actual decayed counts without overlapping sums. Healthy promotion compares qualified common metric pairs against a fixed incumbent; expiry alone is not a performance advantage. `ordinarySwitch` excludes first choice/trials/Peek and is distinct from `switchFlap`. Do not add per-packet logs or exported target identities.
