...
- `src/node.rs` — `Node` (all per-protocol fields, incl. **ECH**: `ech_enabled` / `ech_config` / `ech_config_path`) **plus `Group` and `GroupPolicy`**. `Node::derive_id()` is the single node-identity source: a UUID v5 over `protocol|host|port|credential-fingerprint` (credentials resolved exactly like the protocol handlers resolve them; name and transport/obfs knobs excluded), assigned at every construction entry (share links, dae parser, subscriptions, structured-format config loads) so reloads and subscription refreshes keep health state, latency history, and session pools. `Node::default()` yields a **nil** ID — the runtime registry rejects it, so a missed construction entry fails loudly. `Config::validate` rejects two different-named nodes deriving the same ID (identical endpoint). (Selector/URLTest/LoadBalance/Fallback). `src/group.rs` is a 2-line re-export. `Group.groups: Vec<String>` holds nested sub-group tags; `Group.default`, `final_outbound` (dae `final:`), `tolerance` (default 50 ms), `idle_timeout`, `interrupt_connections`, `check_url` (per-group health check target, sing-box urltest `url`).
...
- `src/runtime.rs` — **per-node runtime ownership** (`OutboundRuntimeRegistry`, ControlPlane is the single owner): `Node.id` → `NodeRuntime` (immutable node config, `OutboundCapabilities`, `ProtocolRuntime`). Built/validated at startup and rebuilt on reload (nil/duplicate UUIDs abort); a reload **reuses the previous generation's `NodeRuntime` for nodes whose full config (minus parse timestamps) is unchanged** — the content-derived ID alone is too narrow (it excludes SNI/transport/obfs) — so QUIC connections and AnyTLS sessions survive unchanged-node reloads; the transfer is recorded on the old generation (`moved_out`) only at the reload commit point, and the retiring generation skips exactly those runtimes at drain/shutdown (an aborted reload transfers nothing). Publication makes the old generation terminal to new warm/speculative work, while its DNS runtime keeps a pinned reference; after old DNS leases/transports retire, session pools reject new opens and drain live streams instead of cutting Ready UDP endpoints or TCP relays. Process shutdown force-closes pools and QUIC clients (`QuicRuntimeClient::force_close` → `QuicClient::force_close`) only after the global flow drain. `acquire_dial_permit` acquires both the generation-local configured limit and one immutable process-wide descriptor gate shared across overlapping reload generations, so a changed local limit applies immediately without double-spending old in-flight permits.
- `src/descriptor.rs` — the per-protocol facts table (`ProtocolDescriptor`: `capabilities` as `fn(&Node) -> OutboundCapabilities` — node-aware because trojan/anytls gate UDP on `node.network` at dial time; `network_allows_udp` is the single gate both sides share — `pool_ready_streams`/`pool_bare_tcp` as `fn(&Node) -> bool`, generation-runtime kind, share-link schemes) consumed by `OutboundCapabilities::for_node`, the runtime-registry build, and the pool-gating call sites. `ProxyRegistry::default_resolver` validates slot/table parity at assembly (packet slot vs default-node `udp`, warmable slot vs generation runtime; Block's packet slot is an explicitly annotated exemption) and panics on inconsistency.
- `src/proxy/mod.rs` — the handler surface is split into narrow capability traits: `TcpOutbound` (`dial`/`dial_with_tcp`/`dial_runtime`), `PacketOutbound` (`dial_udp_transport` → `PacketTransport`, the only UDP contract, plus generation-pinned and speculative variants), `WarmableOutbound` (`warm_udp`), and `ProbeableOutbound` (`test_connectivity`). `ProxyRegistry` stores one `ProtocolEntry` per protocol — the descriptor plus the capability objects it implements (`None` slot = no such capability; the registry refuses those dispatches) — while session ownership stays in the runtime registry; `ProxyStream::into_tcp_stream` preserves the zero-copy splice downcast invariant.
...
- `src/control/` — the control plane:
  - `mod.rs` — `ControlPlane`: TPROXY TCP/UDP v4+v6 accept loop, `ControlCommand` mpsc channel (live commands: `ReloadConfig`, `MergeSubscription`, `GetStats`, `Shutdown`), immutable startup descriptor budget driving independent TCP/UDP/DNS admission semaphores (`try_acquire` — drop at capacity, never hold the fd), listener-fd publication to `LISTEN_SOCKET_MAP`.
  ...
  - `reload.rs` — `apply_runtime_config` is the single rebuild pipeline for SIGHUP reload and subscription merge. It epoch-cancels and drains only UDP initializers before swapping while preserving Ready endpoints; generation-safe cleanup cannot remove a replacement. It also owns opt-in UDP warm coordination: after startup **and after every probe cycle**, each configured group's top-N (N = `udp_warm_node_count`, capped at 3) latency-ordered, UDP-capable leaves are re-selected and dispatched at most four at once, so freshly measured fast nodes get pre-dialed transports before they win a selection; a process-wide cap of 4×N re-ranks the merged candidates by global UDP latency and truncates, so the group count cannot inflate retained transports; already-warm transports return `AlreadyReady` (cheap no-op).
  ...
  - `probers.rs` — `ProxyHttpProber` (HTTP through the node, 200–499 healthy), `ProxyUdpProber` (one DNS query through `dial_udp_transport`). Both reuse the node's generation runtime only when it already holds warm session state (`NodeRuntime::has_warm_resources`); cold nodes dial through an ephemeral one-shot runtime that the prober closes deterministically after the probe (`probe_dial`/`close_ephemeral`) — an abandoned ephemeral AnyTLS pool would keep its demux-held sessions open forever. Cold probes therefore measure cold-start latency, warm probes the hot path.
...
- Interface modes: omitting `global.lan_interface` installs no LAN hooks (there is no implicit `lo` fallback); configured `wan_interface` ingress/egress hooks still proxy host-originated TCP/UDP, including WAN bond slaves discovered at startup or by `IfaceWatcher`. `dae0`/`daens`, transparent listeners, cgroup hooks, and bypass-mark handling remain required in WAN-only mode.
