...
- **HTTP APIs:** axum 0.8 + tower-http 0.7. Default-on `clash-api` enables WebSockets; independent default-off `native-api` owns Hyper HTTP/1.1 and observation, config, probe, DNS, lifecycle and M9 source-owned management. Native limits: 64 connections, 5s headers, 30s idle/stalled-write deadlines, 5s joined HTTP shutdown; one 1s sampler. Flow recorder1024/64steps/8MiB including snapshots, terminal300s; flow snapshots8/30s. Node snapshots8/30s/4MiB. SSE512events/60s/16clients/64queue/15sheartbeat, events replay before ready on valid resume; logs ready before replay. Native group IDs are process-stable random identities, not positional config UUIDs. Per-network Selector, source-owned Group PATCH, synchronous main-source node/provider create/delete and verified-geodata operations are enabled; automatic overrides and native runtime_mode remain gated. Default-off `native-ui` embeds pinned real doona assets; matching source/provenance/notices live under `crates/honk-core/assets/`. `alive/observations.rs` retains qualified actual probe facts independently of legacy routing liveness; never infer fresh native health from restored/synthetic/derived collections. Config/auth: `.agents/rules/configuration.md`, `.agents/rules/security.md`.
