...
- `src/quic.rs` — shared quinn 0.11 plumbing for tuic/juicity/hysteria2: `client_config(node, ...)` (async — may run ECH discovery) assembling a quinn ClientConfig over the **BoringSSL crypto backend** (ALPN, cubic/new_reno/bbr), client `Endpoint` on `SO_MARK`'ed UDP sockets, single-flight `QuicClient<C>` connection holder, `QuicBiStream`, plus `#[cfg(test)] testutil` in-process QUIC servers (rustls, for interop coverage).
...
- `src/urltest.rs` — on-demand latency measurement backing the clash API delay endpoints: dials the check URL through the node's handler (real TLS handshake for https via `tls::build_http1_connector` — HTTP/1.1-only ALPN even in Chrome mode, otherwise h2-capable servers like gstatic break the HTTP/1.1 `HEAD` probe and every measurement fails), `HEAD /`, status 200–499 OK; group measurement is concurrent (cap 10); **failures clear the node's latency history** so it sorts last. Empty URL normalizes to `https://www.gstatic.com/generate_204`.
