...
- `src/proxy/uot.rs` + `src/proxy/vless_mux.rs` — shared UoT v2 framing and sing-box H2MUX. H2MUX targets `sp.mux.sing-box.arpa:444`, backend 2, carries logical TCP and native connected UDP, and optionally adds sing-mux v1 padding. Each node has at most two reusable or dialing carriers × 128 streams; a draining carrier can overlap its replacement until its live streams finish. Saturation backpressures, GOAWAY rolls new work, driver failure fans out, and speculative losers never publish.
- `src/proxy/vless_cool.rs` — Xray VLESS mux command with logical TCP, Single/pooled XUDP framing, full-cone reply metadata, one ordered carrier writer, nonblocking per-stream dispatch, monotonically allocated session IDs, and a two-active-carrier × 128-stream pool. Draining carriers may overlap replacements until their live streams finish. Single XUDP uses reserved global id 0 and no pool. No VLESS mode probes, falls back, or replays a first UDP packet.
...
- `src/alive/` — `AliveDialerSet` health checking. All per-node state is keyed by **NodeId** (`Uuid`): states/collections/registrations/probe history/emergency triggers, `register_node`, `notify_check_*`, `report_*_traffic`, `record_dial_failure`; the registered name rides along for logs and the prober's node lookup, and the death callback carries `(NodeId, name)`. Group tables stay name-keyed (groups have no NodeId), and so does the per-(member tag, check_url) state — a member may be a sub-group (sing-box RealTag). Split: `mod.rs` (state, thresholds, registries, eBPF connectivity-push callback, `StickyCache`), `probe.rs` (`probe_node` HTTP/raw-connect, `probe_node_udp` DNS-through-`dial_udp_transport`, concurrent cycle runner), `collection.rs` (`DialerCollection`: latencies + moving average + alive flag; probe failures update only liveness/cooldown, while real dial failures append a synthetic 10s display-only sample and add a strike; strikes demote the node until `max(strikes, 2)` consecutive real successes clear them), `latencies.rs` (O(1) ring buffer, cap 10; samples carry measurement `SystemTime` — clash history renders real times, and `last_real_sample()` filters synthetic entries so dashboards never show a bogus 10000ms).
  ...
  - UDP probe (injected by honk-core via `set_udp_probe`): one minimal DNS query to the first `global.udp_check_dns` target (default 8.8.8.8:53) through the node's own `dial_udp_transport`; success marks **both** UDP domains alive with the measured RTT, failure adds one probe failure per UDP domain. UDP probes never touch TCP state and vice versa. With `honk-policy`, the same cycle separately opens a packet transport for each node belonging to a Honk group and performs a real TLS-in-QUIC handshake with ALPN `h3` to the first HTTPS `global.tcp_check_url`; its exact-target `DataUdp` outcome trains the Honk scorer but never changes DNS-based liveness or awards unobserved byte volume. Missing or non-HTTPS check URLs disable only this score probe. Established endpoint send/receive/reply-idle errors add a DataUdp traffic failure, while intentional endpoint retirement and shutdown are health-neutral. `has_udp_state(node)` distinguishes "never UDP-probed" from "UDP-probed and dead". Per-group custom check URLs are probed and tracked separately: `(member tag, check_url)` state (TCP-only, 3-failure death, same backoff/recovery) via `sync_group_check_urls` — a member may be a sub-group whose current leaf is dialed.
...
- `honk-policy` — forwards the default-off reliability-first group scorer into `honk-outbound` and `honk-config`. It enables `policy: honk`, keeps bounded memory-only target scores across reload, and adds no runtime knobs or default behavior changes.
