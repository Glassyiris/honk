...
- **Allocator:** the shipped binary (`honk-core` bin) uses **mimalloc** as the global allocator behind the default-on `mimalloc` cargo feature (musl's stock malloc is slow under contention). mimalloc reserves aligned 1 GiB arenas and decommits purged pages, but fragment-pinned worker heaps linger. `main.rs` therefore builds Tokio explicitly, dispatches the top-level application future onto a worker, enables mimalloc's cross-thread abandoned-page reclaim before starting the runtime, and runs `mi_collect(true)` from each owning worker's park callback with an OS-thread-local 60s cooldown and a delayed first collection. A bounded periodic rendezvous, attempted only when every other worker is idle, wakes owners that stayed parked after cross-thread frees so the same callback can purge their heap. `HONK_MI_COLLECT_SECS=0` installs neither the hook nor the rendezvous. The clash `/logs` tracing layer (`clash_api/logs.rs`) skips formatting entirely when it has no subscribers — it is unfiltered, so without the check every sub-level event in the data path would cost a `String`.
