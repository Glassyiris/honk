...
- `src/control/` — the control plane:
  - `mod.rs` — `ControlPlane`: TPROXY TCP/UDP listeners plus the supervised NFQUEUE runtime, fixed startup descriptor budgets, listener-FD publication, token-exhaustion ring/backstop handling, and ordered startup/shutdown. Queue/listener/verdict/token faults are fatal branches of `run`, never detached background warnings.
...
- `src/mode.rs` — `DatapathFlagsCoordinator` is the sole owner of `ModeState` and `DATAPATH_FLAGS_MAP`; it serializes initialize/mode/GLOBAL/static updates with NFQUEUE fence/reopen/disable so a concurrent API update cannot republish ready during reload. Fence acknowledgement follows READY=false publication, a kernel reader-epoch grace period, and removal of every undelivered Preparing/Pending state.
...
- `src/clash_api.rs` + `clash_api/{logs,doh,ui}.rs` — Clash-compatible REST/WS API and external UI. `GET /stats` returns the userspace snapshot (not eBPF `OUTBOUND_STATS`). Mode and GLOBAL selection mutations go through `DatapathFlagsCoordinator`, so reload fencing and the latest mode/static bits compose atomically; selector mutations remain group-manager operations.
...
- **NFQUEUE readiness and ownership:** `DATAPATH_FLAG_NFQ_ENABLED && !DATAPATH_FLAG_NFQ_READY` drops only new flows that require staging. The sole coordinator clears readiness, flips `UDP_DECISION_EPOCH`, waits for the old per-CPU `UDP_DECISION_INFLIGHT` slot, and removes residual Preparing/Pending states before acknowledging a reload/shutdown fence; delayed queue delivery then fails token lookup. honk exclusively owns queue `320` and exact nftables `inet honk_nfqueue` / `udp_decision`; no bypass/fanout/fail-open is allowed, and same-netns firewall managers must not mutate those names while honk runs.
