...
- Environment variables: `RUST_LOG`, `HONK_UI_DOWNLOAD_URL` (UI zip override), `HONK_POOL_DISABLE=1` (bypass connection pool), `HONK_MI_COLLECT_SECS` (mimalloc builds only: periodic full arena collect interval, default 60s, `0` disables; first collect is delayed one period and runs on a blocking thread).
