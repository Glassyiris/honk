...
- `src/proxy/uot.rs` + `src/proxy/vless_mux.rs` — shared UoT v2 framing and sing-box H2MUX. H2MUX targets `sp.mux.sing-box.arpa:444`, backend 2, carries logical TCP and native connected UDP, and optionally adds sing-mux v1 padding. Each node has at most two carriers × 128 streams; saturation backpressures, GOAWAY rolls new work, driver failure fans out, and speculative losers never publish.
- `src/proxy/vless_cool.rs` — Xray VLESS mux command with logical TCP, Single/pooled XUDP framing, full-cone reply metadata, one ordered carrier writer, nonblocking per-stream dispatch, monotonically allocated session IDs, and a two-carrier × 128-stream pool. Single XUDP uses reserved global id 0 and no pool. No VLESS mode probes, falls back, or replays a first UDP packet.
...
- `src/session.rs` — unified node-owned session pool for AnyTLS, VLESS H2MUX, and VLESS Mux.Cool (QUIC keeps `quic::QuicClient`): hard session caps, least-loaded scheduling, `Active`/`Draining`/`Closed`, atomic permits, pool-owned single-flight dials, bounded retry classification, backoff, retirement, and idempotent force shutdown. Cold speculative VLESS dials use caller-owned provisional slots; cancellation drops the physical dial and only winner commit publishes.
- AnyTLS spreads across two physical sessions; H2MUX and Mux.Cool fill the least-loaded carrier, bounded at two × 128 streams. Selector/UDP warm bits pin a pool until the final owner releases it.
