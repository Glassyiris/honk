...
- **Allocator:** the shipped binary (`honk-core` bin) uses **mimalloc** as the global allocator behind the default-on `mimalloc` cargo feature (musl's stock malloc is slow under contention). mimalloc reserves aligned 1 GiB arenas and decommits purged pages, but fragment-pinned worker heaps linger. `main.rs` therefore builds Tokio explicitly, dispatches the top-level application future onto a worker, and runs `mi_collect(true)` on each owning worker when it parks and from a bounded periodic owner rendezvous that reaches workers remaining parked after cross-thread frees. Both paths share an OS-thread-local 60s cooldown and delay their first collection; `HONK_MI_COLLECT_SECS=0` installs neither path. RSS can still read as a traffic high-water mark under THP. The clash `/logs` tracing layer (`clash_api/logs.rs`) skips formatting entirely when it has no subscribers — it is unfiltered, so without the check every sub-level event in the data path would cost a `String`.
