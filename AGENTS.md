...
- **HTTP APIs:** axum 0.8 + tower-http 0.7. Default-on `clash-api` enables WebSockets; independent default-off `native-api` owns Hyper HTTP/1.1 plus `native_api/{types,security,ui,observation,flows,catalog,events}.rs`. Native limits: 64 connections, 5s headers, 30s idle/stalled-write deadlines, 5s joined shutdown; one 1s sampler. Flow recorder1024/64steps/8MiB including snapshots, terminal300s; flow snapshots8/30s. Node snapshots8/30s/4MiB. SSE512events/60s/16clients/64queue/15sheartbeat, replay before ready on valid resume. Native group IDs are process-stable random identities, not positional config UUIDs. Group writes stay gated. `alive/observations.rs` retains qualified actual probe facts independently of legacy routing liveness; never infer fresh native health from restored/synthetic/derived collections. Config/auth: `.agents/rules/configuration.md`, `.agents/rules/security.md`.
