...
- Handlers: `direct` (bypass-marked dial; UDP `relay_addr` **is the target**), `block`, `socks5` (CONNECT + UDP ASSOCIATE — `relay_addr` is the server-assigned relay, **no loopback bridge**), `shadowsocks` (+ `shadowsocks_2022.rs` SIP022: BLAKE3 KDF, EIH, replay protection; selected via `is_2022_method`), `ssr` (TCP only), `trojan` (+ UDP associate bridge), `trojan_go` (own smux-style mux, TCP only — collision-checked u16 ids, 128 streams per mux, max 2 muxes/key, HOL-stall closes the mux, cancel-safe header guard), `vmess` (TCP only), `vless` (TCP only), `anytls`, `tuic`, `juicity`, `hysteria2/`.
