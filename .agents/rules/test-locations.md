...
- Runtime and transport:
    ...
    - `crates/honk-core/tests/native_api_test.rs` — independent native listener security, limits, read projections, UI hosting, connection ownership and shutdown. Live TCP accounting, prefix failures, native-only routing evidence and consumer transitions stay in the relay/control/tracker owners; native snapshot/sampler boundaries stay with `native_api`.
